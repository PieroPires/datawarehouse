...
command <- paste("SELECT CLASSIFICACAO_2014 AS id_empresa,",
                 " REPLACE(EMPRESA ,' ','%20') AS nome_empresa",
                 " FROM VAGAS_DW.TMP_1000_MAIORES_EMPRESAS ",
                 " WHERE (FLAG_PROCESSADO IS NULL ",
                 " OR FLAG_PROCESSADO = 0) ORDER BY ID ASC",sep = "")
#print(command)
...
#       
      ...
      if (exists("results") == TRUE){
        ...
        if(length(results$responseData$results) == 0 || error == TRUE){
          if (error == FALSE) { 
