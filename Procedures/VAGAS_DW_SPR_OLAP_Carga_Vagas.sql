...
ATINGIU_LIMITE_CANDIDATURAS,NAV_EXC,CAPTACAO_CONTINUA,EXIBE_VAGAS_COM,INVISIVEL,TIPO_PROCESSO,FLAG_VAGA_TESTE, CNAE_SECAO_ID, CNAE_SECAO, CNAE_DIVISAO_ID, CNAE_DIVISAO, CNAE_CLASSE_ID, CNAE_CLASSE, CNAE_FAIXA_FUNCIONARIOS,CNAE_SUBCLASSE_ID_C, CNAE_SUBCLASSE_DESCR_C,REGIAO, QTD_MENSAGENS_UNICAS, COD_FUNC )
SELECT * FROM VAGAS_DW.TMP_VAGAS
...
SET	QTD_MENSAGENS_UNICAS = (SELECT	COUNT(DISTINCT A1.CodCand_hist) AS QTD_MENSAGENS_UNICAS
							FROM	[hrh-data].[dbo].[Historico] AS A1
							WHERE	A.VAGAS_Cod_Vaga = A1.CodVaga_hist
									AND A1.Tipo_hist IN (-120, -107)
									AND A1.Anotacao_hist LIKE '%foi%aprovado%'
									AND A1.ChaveSQL_hist >= 1322983768)
FROM	[VAGAS_DW].[VAGAS_DW].[VAGAS] AS A	INNER JOIN [hrh-data].[dbo].[Historico] AS B ON A.VAGAS_Cod_Vaga = B.CodVaga_hist
WHERE	(A.VAGA_ATIVA = 'SIM'
		 OR DATA_EXPIRACAO >= DATEADD(DAY, -3, CAST(GETDATE() AS DATE)))
		AND B.Tipo_hist IN (-120, -107)
		AND B.Anotacao_hist LIKE '%foi%aprovado%'
		AND B.ChaveSQL_hist >= 1322983768