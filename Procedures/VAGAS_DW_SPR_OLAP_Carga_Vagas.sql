...
-----------------------------------------------------------
	...
	UPDATE	[VAGAS_DW].[VAGAS]
	SET		ELIMINOU_CANDIDATOS = 'SIM' ,
			QTD_CANDIDATOS_ELIMINADOS = B.QTD_CANDIDATOS_ELIMINADOS ,
			QTD_VAGAS_ELIMINOU_CAND = 1
	FROM	[VAGAS_DW].[VAGAS] AS A		INNER JOIN #TMP_ELIMINOU_CAND_VAGA AS B ON A.VAGAS_Cod_Vaga = B.COD_VAGA ;
	...
	UPDATE	[VAGAS_DW].[VAGAS]
	SET		AVALIOU_CANDIDATOS = 'SIM',
			QTD_AVALIACAO_CANDIDATOS = B.QTD_AVALIACAO_CANDIDATOS
	FROM	[VAGAS_DW].[VAGAS] AS A		INNER JOIN #TMP_AVALIOU_CAND_VAGA AS B ON A.VAGAS_Cod_vaga = B.COD_VAGA ;
	...
	FROM	[VAGAS_DW].[VAGAS] AS A		LEFT OUTER JOIN #TMP_VAGA_FECHADA_CANDIDATO AS B ON A.VAGAS_Cod_Vaga = B.COD_VAGA
