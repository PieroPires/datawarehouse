...
-- Apaga os registros da tabela fato que existem na stage:
DELETE FROM [VAGAS_DW].[LINKS_TRANSICAO_STATUS]
FROM [VAGAS_DW].[LINKS_TRANSICAO_STATUS] AS Links
WHERE EXISTS (SELECT * FROM [stage].[VAGAS_DW].[TMP_LINKS_HOMEPAGES_TRANSICAO] AS Links_A1
			  WHERE	Links.ID_LINK = Links_A1.ID_LINK) ;
...
FROM	
	(
		...
		SELECT	A.ID_LINK ,
				...
				END AS DATA_VALOR_NORMALIZADO
		FROM	[STAGE].[VAGAS_DW].[TMP_LINKS_TRANSICAO_STATUS] AS A
	) AS TAB
ORDER BY
		1 ASC ;
...
FROM	#INTERVALO_HORAS_UTEIS AS A	INNER JOIN STAGE.VAGAS_DW.TMP_LINKS_TRANSICAO_STATUS AS B ON A.ID_LINK = B.ID_LINK
