...
TRUNCATE TABLE VAGAS_DW.TMP_CLIENTES

DROP INDEX IF EXISTS IDX_TMP_CLIENTES_CONTA_ID ON VAGAS_DW.TMP_CLIENTES
...
SELECT A.Ident_Cli AS Cliente_VAGAS,
	   ...
	   (SELECT	COUNT(*) AS QTD_LOGIN_ULT_30_DIAS
		...
		( SELECT	COUNT(*) AS QTD_FUNC_REMOVIDO
		  FROM		[hrh-data].[dbo].[Funcionarios] AS A1
		  WHERE		A.Cod_cli = A1.CodCli_func
					AND A1.Removido_func = 1 ) AS QTD_FUNC_REMOVIDO
INTO #TMP_PERFIL_USO
...
INSERT INTO VAGAS_DW.TMP_CLIENTES (DATA_REFERENCIA,DATA_REFERENCIA_12_MESES_INICIO,DATA_REFERENCIA_12_MESES_FIM,COD_CLI,CLIENTE_VAGAS,
...
SELECT CONVERT(SMALLDATETIME,CONVERT(VARCHAR,GETDATE(),112)) AS DATA_REFERENCIA,
	   ...
	   replace(ltrim(rtrim(CONTA_ID)), char(9), '') as CONTA_ID,
	   UF,
	   CIDADE,
	   POSSUI_LOGO,
...
CREATE NONCLUSTERED INDEX IDX_TMP_CLIENTES_CONTA_ID ON VAGAS_DW.TMP_CLIENTES (CONTA_ID)
