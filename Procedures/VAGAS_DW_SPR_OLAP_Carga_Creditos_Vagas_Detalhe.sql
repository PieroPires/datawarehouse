...
TRUNCATE TABLE [STAGE].[VAGAS_DW].[TMP_CREDITOS_VAGAS_DETALHE] ;
...
WITH Compras_FIT (DATA, CLIENTE, TIPO, SEGMENTACAO, DESCRICAO, VALOR) AS (
	...
	FROM [VAGAS_DW].[VAGAS_DW].[CLIENTES] AS A		CROSS APPLY (SELECT TOP 1 A1.DataFechamento ,
																 ...
																 WHERE A.CONTA_ID = A1.CONTAID
																	...
																	AND A1.FASE = 'fechado_e_ganho'
																 ORDER BY A1.DataFechamento DESC, A1.DataCriacao DESC) AS B -- Clientes FIT que pediram rescisao
													INNER JOIN [STAGE].[VAGAS_DW].[TMP_CREDITOS_LANC] AS C ON A.Cod_cli = C.COD_CLI
...
SELECT DATA ,
	TIPO ,
	'Pacotes de créditos comprados' + ' ' + '-' + ' ' + CONVERT(VARCHAR(15), DESCRICAO) AS SEGMENTACAO,
	...
	CASE WHEN SEGMENTACAO IN ('CONVERSÃO DE CRÉDITOS', 'CRÉDITOS DE CORTESIA') THEN VALOR ELSE 1 END AS VALOR
INTO #COMPRA_PACOTE
...
FROM [STAGE].[VAGAS_DW].[TMP_CREDITOS_CONSUMO]	INNER JOIN ( SELECT DISTINCT CLIENTE, TIPO FROM #COMPRAS_MODALIDADE 
...
INSERT INTO [STAGE].[VAGAS_DW].[TMP_CREDITOS_VAGAS_DETALHE](DATA_REFERENCIA, DATA, TIPO, SEGMENTACAO, DESCRICAO, VALOR)
...
INTO #TMP_TIPO 
FROM [STAGE].[VAGAS_DW].[TMP_CREDITOS_VAGAS_DETALHE] ;
...
WITH LAYOUT_PRINCIPAL AS (
	...
	SELECT TIPO ,
		...
		DESCRICAO
	FROM LAYOUT_PRINCIPAL CROSS JOIN #TMP_DESCRICAO
	WHERE SEGMENTACAO IN ('Pacotes de créditos comprados - ONLINE/EMPRESA')
	...
	SELECT TIPO ,
		...
		DESCRICAO
	FROM LAYOUT_PRINCIPAL CROSS JOIN #TMP_DESCRICAO
	WHERE SEGMENTACAO IN ('Pacotes de créditos comprados - OFFLINE/MANUT')
		AND DESCRICAO IN ('1', '2', '3', '5', '10', 'OUTROS', 'Total de créditos convertidos', 'Total de créditos cortesia')
...
WITH CTE_DATAS AS (
	SELECT MIN(A.DATA) AS MENOR_DATA ,
		...
		B.TIPO
	FROM [STAGE].[VAGAS_DW].[TMP_CREDITOS_VAGAS_DETALHE] AS A CROSS JOIN #TMP_LAYOUT AS B
...
SELECT * FROM [STAGE].[VAGAS_DW].[TMP_CREDITOS_VAGAS_DETALHE] ;
