...
INSERT INTO VAGAS_DW.TMP_VAGAS (VAGAS_Cod_Vaga,COD_CLI,CLIENTE,CARGO,ESCOLARIDADE,NIVEL,CIDADE,UF,PREF_SEXO,AREA_01,AREA_02,AREA_03,ACEITA_CAND_OUTRA_REGIAO,VAGA_VALIDADA,DATA_CADASTRAMENTO_SOURCE,DATA_CADASTRAMENTO,DATA_VALIDACAO_SOURCE,DATA_VALIDACAO,DATA_PUBLICACAO_SOURCE,DATA_PUBLICACAO,ACEITA_CAND_OUTRO_NIVEL,ACEITA_CAND_OUTRA_AREA,DISPONIB_VIAGEM,DATA_EXPIRACAO_SOURCE,DATA_EXPIRACAO,DATA_ULT_TRIAGEM_SOURCE,DATA_ULT_TRIAGEM,SOLICITA_PREENCHIMENTO_FICHA,PCD,ANUNCIO_IDENTIFICADO,SEGMENTO,GRUPO_SEGMENTO,QTD_POSICOES,QTD_DIAS_ALERTADO,QTD_ALERTA_DISPARADO,PERC_RETORNO,QTD_PageViews,PAIS,DATA_ATUALIZACAO_SOURCE,DATA_ATUALIZACAO,VEICULACAO_SUSPENSA,CLIENTE_BLOQUEADO,ATINGIU_LIMITE_CANDIDATURAS,NAV_EXC,CAPTACAO_CONTINUA,EXIBE_VAGAS_COM,INVISIVEL,TIPO_PROCESSO,FLAG_VAGA_TESTE,REGIAO,COD_FUNC,DIVISAO,SOLICITA_TESTE,TESTE_OBRIGATORIO,VAGAS_ETALENT,INDICACAO_VAGA,POSSUI_TESTE_CUSTOMIZADO,POSSUI_FICHA_COMPLEMENTAR,NOME_FUNC,ORIGEM_VAGA_MODELO,QTD_FICHAS_VAGA,QTD_FICHAS_COMPLEMENTAR,Cod_publicacaoVaga,COD_CIDADE,Vaga_temporaria)
SELECT A.Cod_Vaga VAGAS_Cod_Vaga,
	...
	0 AS QTD_DIAS_ALERTADO,
    0 AS QTD_ALERTA_DISPARADO,
	0 AS PERC_RETORNO,
	...
	Q.Nome_func AS NOME_FUNC,
	IIF(A.CodVagaModeloBase_vaga IS NULL, 'NÃO', 'SIM') AS ORIGEM_VAGA_MODELO,
	IIF(A.CodFicha1_vaga IS NULL, 0, 1)+IIF(A.CodFicha2_vaga IS NULL, 0, 1)+IIF(A.CodFicha3_vaga IS NULL, 0, 1)+IIF(A.CodFicha4_vaga IS NULL, 0, 1) AS QTD_FICHAS_VAGA,
	IIF(P1.Ident_fic LIKE '%complementar%', 1, 0)+IIF(P2.Ident_fic LIKE '%complementar%', 1, 0)+IIF(P3.Ident_fic LIKE '%complementar%', 1, 0)+IIF(P4.Ident_fic LIKE '%complementar%', 1, 0) AS QTD_FICHAS_COMPLEMENTAR,
	A.codpublicacao_vaga AS Cod_publicacaoVaga,
	A.Codcidade_vaga AS COD_CIDADE,
	iif(A.cargo_vaga like '%temporari[oa]%', 'Sim', 'Não') as Vaga_temporaria
...
LEFT OUTER JOIN [hrh-data].[dbo].[Funcionarios] AS Q ON A.CodFunc_vaga = Q.Cod_func
