...
--------------------------------------------------------------
UPDATE	[VAGAS_DW].[TMP_REUNIOES]
SET		CONTAID = B.CONTA_ID ,
		CONTA = B.CONTA_CRM
FROM	[VAGAS_DW].[TMP_REUNIOES] AS A		LEFT OUTER JOIN ( SELECT A1.CONTA_ID ,
																	 A1.CONTA_CRM
...
SET		CONTAID = B.CONTAID ,
		CONTA = B.CONTA
FROM	[VAGAS_DW].[TMP_REUNIOES] AS A		LEFT OUTER JOIN [VAGAS_DW].[OPORTUNIDADES] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.OportunidadeID
...
SET		CONTAID = B.COD_CONTA_CRM ,
		CONTA = B.CONTA_CRM
FROM	[VAGAS_DW].[TMP_REUNIOES] AS A		LEFT OUTER JOIN [VAGAS_DW].[CONTATOS_CRM] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.COD_CONTATO
...
SET		CONTAID = B.CONTA_ID ,
		CONTA = B.CONTA
FROM	[VAGAS_DW].[TMP_REUNIOES] AS A		LEFT OUTER JOIN [VAGAS_DW].[CASOS] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.ID_CASO
