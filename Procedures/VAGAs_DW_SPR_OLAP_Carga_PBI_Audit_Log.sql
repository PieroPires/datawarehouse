...
WHERE	CONVERT(DATE, A.CREATIONTIME) = (SELECT CONVERT(DATE, MAX(A1.CREATIONTIME))
										 FROM	[VAGAS_DW].[PBI_AUDIT_LOG] AS A1) ;
...
SELECT	A.ID ,
		...
		A.DATASETNAME ,
		A.REPORTNAME 
FROM	[STAGE].[VAGAS_DW].[TMP_PBI_AUDIT_LOG] AS A
WHERE	CONVERT(DATETIME, A.CREATIONTIME, 103) > (SELECT ISNULL(CONVERT(DATE, MAX(A1.CREATIONTIME)), '19000101')
												  FROM	[VAGAS_DW].[PBI_AUDIT_LOG] AS A1) ;