...
SET @CMD = ' BULK INSERT STAGE.VAGAS_DW.TMP_EQUIPES 
...
SELECT REPLACE(NOME,'"','') AS NOME, 
	   ...
	   CICLO_5,
	   CHAVE_EQUIPE
FROM STAGE.VAGAS_DW.TMP_EQUIPES A
WHERE TIPO <> '0' 
AND TIPO <> 'Categoria'
...
INNER JOIN STAGE.VAGAS_DW.TMP_EQUIPES B ON REPLACE(B.NOME,'"','') = REPLACE(A.NOME,'"','') 
...
UPDATE VAGAS_DW.EQUIPES SET CICLO_ATUAL = CASE WHEN LINK_CICLO_5 IS NOT NULL THEN 5 
...
FROM VAGAS_DW.EQUIPES A
WHERE ATIVO = 1 
AND NOT EXISTS ( SELECT * FROM STAGE.VAGAS_DW.TMP_EQUIPES
					WHERE REPLACE(NOME,'"','') = REPLACE(A.NOME,'"','')
