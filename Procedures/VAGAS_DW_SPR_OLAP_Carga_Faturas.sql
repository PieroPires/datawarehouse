...
WHERE DATA_REFERENCIA = ( SELECT MAX(DATA_REFERENCIA) 
						  FROM VAGAS_DW.FATURAS
						  WHERE COD_CLI_CRM = A.COD_CLI_CRM ) -- ÚLTIMA DATA REF.
AND EXISTS ( SELECT * 
...
AND ( SELECT COUNT(*) 
	  FROM VAGAS_DW.FATURAS 
	  WHERE COD_CLI_CRM = A.COD_CLI_CRM
...
VALOR,JUROS,VALOR_LIQUIDO,TIPO_ORIGEM,TIPO,SALDO,STATUS_ORIGEM,STATUS,ID_CNAB,DATA_VENCIMENTO_REAL_ORIGINAL,FAIXA_ATRASO,DIAS_ATRASO,
DATA_REFERENCIA,FLAG_PAGO,FLAG_PAGO_INTEGRAL,FLAG_PAGO_EM_ATRASO,DATA_EMISSAO,PRAZO_PAGTO,STATUS_INADIMPLENCIA,CNPJ
HAVING COUNT(*) > 1
ORDER BY CLIENTE
