...
WHERE EXISTS ( SELECT 1 FROM [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM]
				WHERE ID_LIGACAO = A.ID_LIGACAO )

-- var Grupo
UPDATE [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] SET GRUPO_VENDEDOR = B.GRUPO_VENDEDOR
FROM [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] A
INNER JOIN VAGAS_DW.GRUPO_VENDEDORES B ON B.VENDEDOR = A.USUARIO_RESPONSAVEL

---- CLIENTE ASSOCIADO À OPORTUNIDADE
--UPDATE [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] SET CONTA = B.CONTA
--FROM [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] A
...
--UPDATE	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM]
--SET		CONTA_ID = B.CONTAID
--FROM	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] AS A		INNER JOIN [VAGAS_DW].[OPORTUNIDADES] AS B ON B.OPORTUNIDADE = A.REFERENTE_A
--WHERE	A.TIPO_CONTATO = 'Opportunities' ;


--UPDATE [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] SET CONTA = A.REFERENTE_A
--FROM [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] A
--WHERE A.TIPO_CONTATO IN ('Accounts','Leads')

--UPDATE	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM]
--SET		CONTA_ID = A.ID_TIPO_CONTATO
--FROM	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] AS A
...
SET		CONTA_ID = B.CONTA_ID ,
		CONTA = B.CONTA_CRM
FROM	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] AS A		LEFT OUTER JOIN ( SELECT A1.CONTA_ID ,
																		 A1.CONTA_CRM
...
SET		CONTA_ID = B.CONTAID ,
		CONTA = B.CONTA
FROM	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] AS A		LEFT OUTER JOIN [VAGAS_DW].[OPORTUNIDADES] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.OportunidadeID
...
SET		CONTA_ID = B.COD_CONTA_CRM ,
		CONTA = B.CONTA_CRM
FROM	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] AS A		LEFT OUTER JOIN [VAGAS_DW].[CONTATOS_CRM] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.COD_CONTATO
...
SET		CONTA_ID = B.CONTA_ID ,
		CONTA = B.CONTA
FROM	[STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM] AS A		LEFT OUTER JOIN [VAGAS_DW].[CASOS] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.ID_CASO
...
SELECT ID_LIGACAO,
	   ...
	   CONTEXTO,
	   ID_CONTEXTO
FROM [STAGE].[VAGAS_DW].[TMP_LIGACOES_CRM]
