...
--UPDATE VAGAS_DW.TMP_EMAILS_CRM SET CONTA_ID = B.CONTAID
--FROM VAGAS_DW.TMP_EMAILS_CRM A
--INNER JOIN VAGAS_DW.OPORTUNIDADES B ON B.OPORTUNIDADE = A.REFERENTE_A
--WHERE A.TIPO_CONTATO = 'Opportunities'
...
--UPDATE	[VAGAS_DW].[TMP_EMAILS_CRM]
--SET		CONTA_ID = A.ID_TIPO_CONTATO
--FROM	[VAGAS_DW].[TMP_EMAILS_CRM] AS A
...
SET		CONTA_ID = B.CONTA_ID ,
		CONTA = B.CONTA_CRM
FROM	[VAGAS_DW].[TMP_EMAILS_CRM] AS A		LEFT OUTER JOIN ( SELECT A1.CONTA_ID ,
																		 A1.CONTA_CRM
...
SET		CONTA_ID = B.CONTAID ,
		CONTA = B.CONTA
FROM	[VAGAS_DW].[TMP_EMAILS_CRM] AS A		LEFT OUTER JOIN [VAGAS_DW].[OPORTUNIDADES] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.OportunidadeID
...
SET		CONTA_ID = B.COD_CONTA_CRM ,
		CONTA = B.CONTA_CRM
FROM	[VAGAS_DW].[TMP_EMAILS_CRM] AS A		LEFT OUTER JOIN [VAGAS_DW].[CONTATOS_CRM] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.COD_CONTATO
...
SET		CONTA_ID = B.CONTA_ID ,
		CONTA = B.CONTA
FROM	[VAGAS_DW].[TMP_EMAILS_CRM] AS A		LEFT OUTER JOIN [VAGAS_DW].[CASOS] AS B ON IIF(ISNULL(A.ID_TIPO_CONTATO, '') = '', A.ID_CONTEXTO, A.ID_TIPO_CONTATO) = B.ID_CASO
