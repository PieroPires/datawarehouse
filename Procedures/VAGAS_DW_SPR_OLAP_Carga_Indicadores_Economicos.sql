...
TRUNCATE TABLE STAGE.VAGAS_DW.TMP_INDICADORES_IPCA
TRUNCATE TABLE STAGE.VAGAS_DW.TMP_INDICADORES_IGPM
TRUNCATE TABLE STAGE.VAGAS_DW.TMP_INDICADORES_DOLAR
...
FROM STAGE.VAGAS_DW.TMP_INDICADORES_IPCA A
...
FROM STAGE.VAGAS_DW.TMP_INDICADORES_IGPM A
...
SELECT	CONVERT(SMALLDATETIME, CONVERT(VARCHAR, '20180901', 112)) AS DATA ,
		...
		CONVERT(FLOAT,REPLACE(REPLACE(indice_acumulado_ano,',','.'),'- ','-')) / 100 AS INDICE_ACUMULADO_ANO_IGPM,
		CONVERT(FLOAT,REPLACE(REPLACE(indice_acumulado_12_meses,',','.'),'- ','-')) / 100 AS INDICE_ACUMULADO_12_MESES_IGPM
FROM	[STAGE].[VAGAS_DW].[TMP_INDICADORES_IGPM] AS A
...
WHERE EXISTS ( SELECT 1 FROM STAGE.VAGAS_DW.TMP_INDICADORES_DOLAR
...
FROM STAGE.VAGAS_DW.TMP_INDICADORES_DOLAR
