...
---- Solução paliativa - Inserção dos valores de Nov/17 manualmente:
...
--SELECT	CONVERT(SMALLDATETIME, CONVERT(VARCHAR, '20171101', 112)) AS DATA ,
--		CONVERT(FLOAT,REPLACE(REPLACE(0.28,',','.'),'- ','-')) / 100 AS INDICE_MES_IPCA,
--		CONVERT(FLOAT,REPLACE(REPLACE(2.4900,',','.'),'- ','-')) / 100 AS INDICE_ACUM_ANO_IPCA,
--		CONVERT(FLOAT,REPLACE(REPLACE(2.8000,',','.'),'- ','-')) / 100 AS INDICE_ACUM_12_MESES_IPCA,
--		CONVERT(FLOAT,REPLACE(REPLACE(REPLACE(REPLACE(indice_mes,',','.'),'- ','-'),'&nbsp;',''),' ','')) / 100 AS INDICE_MES_IGPM,
