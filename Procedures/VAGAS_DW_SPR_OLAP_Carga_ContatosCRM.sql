...
TRUNCATE TABLE [VAGAS_DW].[CONTATOS_CRM] 
 
-- Carrega os dados na VAGAS_DW.CONTATOS_CRM: 
INSERT INTO [VAGAS_DW].[CONTATOS_CRM] (COD_CONTATO, NOME, SOBRENOME, DATA_INCLUSAO, DATA_MODIFICACAO, CARGO, 
DEPARTAMENTO, TELEFONE_CELULAR, TELEFONE_CONTATO, TELEFONE_OUTRO, RUA, CIDADE, ESTADO, PAÍS, EMAIL, CLIENTE_VAGAS, 
CONTA_CRM, COD_CONTA_CRM, TIPO_CLIENTE_MANUT, TIPO_CLIENTE, EX_CLIENTE,CONTATO_PRINCIPAL,ADMIN,FUNCIONARIO_DESLIGADO,DtUltAtividade)
SELECT  A.COD_CONTATO , 
	...
	A.CONTATO_PRINCIPAL,
	A.ADMIN ,
	A.FUNCIONARIO_DESLIGADO ,
	A.DtUltAtividade
FROM  [STAGE].[VAGAS_DW].[TMP_CONTATOS_CRM] AS A  LEFT OUTER JOIN [VAGAS_DW].[CLIENTES] AS B ON A.COD_CONTA_CRM = B.CONTA_ID 
AND B.DATA_REFERENCIA = (SELECT MAX(A1.DATA_REFERENCIA) 
               FROM  [VAGAS_DW].[CLIENTES] AS A1) 
 
 
-- Estrutura temporária:
UPDATE [VAGAS_DW].[CONTATOS_CRM] 
