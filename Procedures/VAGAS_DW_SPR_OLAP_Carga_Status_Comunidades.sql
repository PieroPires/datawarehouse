...
SET NOCOUNT ON
	...
	TRUNCATE TABLE STAGE.VAGAS_DW.TMP_COMUNIDADES_STATUS
	...
	BEGIN 
		SET @MSG = 'ERRO ocorrido na importação dos dados no ID_CONTROLE_SPREADSHEET : 192' 
		RAISERROR(@MSG , 16, 1) 
	END
	...
	SELECT CONVERT(INT,[0]) AS COD_DIV,
		   ...
		   [2] AS SIGLA,
		   [3] AS STATUS
	FROM STAGE.VAGAS_DW.TMP_COMUNIDADES_STATUS
	WHERE [index] > 0
