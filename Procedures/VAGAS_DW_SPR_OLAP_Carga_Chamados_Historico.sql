...
FROM 
	...
	SELECT ID,ID_TICKET,DESCR_HISTORICO,TIPO_HISTORICO,ESTADO,BODY,FILA,TIPO_MSG,ORIGEM,DATA_ENVIO,USUARIO,MSG_PADRAO,PAGINA_ORIGEM,REMETENTE,DESTINATARIO,
			DATA_ALTERACAO,
			CASE WHEN DATEPART(HOUR,DATA_ENVIO) >= 17 -- ESTOU DANDO UMA FOLGA DE 1 HORA ANTES DO EXPEDIENTE PARA QUE CHAMADOS ENTRE A
													  -- AS 17:00 E 18:00 NÃO DISTORÇAM MUITO NEGATIVAMENTE O TMA
							THEN DATEADD(HOUR,8,DATEADD(DAY,1,CONVERT(SMALLDATETIME,CONVERT(VARCHAR,DATA_ENVIO,112))))
							WHEN DATEPART(HOUR,DATA_ENVIO) < 8 
...
UPDATE #TMP_DATA_NORMALIZADA SET DATA_ENVIO_NORMALIZADA = CASE
				...
				WHEN DATEPART(WEEKDAY,DATEADD(DAY,1,DATA_ENVIO_NORMALIZADA)) = 7 THEN DATEADD(DAY,3,DATA_ENVIO_NORMALIZADA)
				-- se o próx. dia for domingo adiantar 2 dias 
				WHEN DATEPART(WEEKDAY,DATEADD(DAY,1,DATA_ENVIO_NORMALIZADA)) = 1 THEN DATEADD(DAY,2,DATA_ENVIO_NORMALIZADA)
				ELSE DATEADD(DAY,1,DATA_ENVIO_NORMALIZADA) END
...
DECLARE @DATA_REFERENCIA_INICIAL SMALLDATETIME,
		...
		@EXTREMIDADE_INFERIOR TINYINT = 8, -- HORÁRIO INICIO DE EXPEDIENTE
