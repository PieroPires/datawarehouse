...
INTO	#TMP_DATA_HORA
FROM	[Export].[Dim].[Data] AS A	CROSS JOIN [Export].[Dim].[Hora] AS B
...
GROUP BY
	...
	SELECT ID_CASO ,
		...
		DATA_PRM_ACAO ,
		DATA_INCLUSAO_NORMALIZADA , 
		(((CASE WHEN (ORIGEM_SUPORTE = 'E-mail' AND (DATA_PRM_ACAO < DATA_INCLUSAO_NORMALIZADA)) THEN 0 ELSE B.DIFERENCA_HORAS END)*60*60)/60/60/24) AS TME ,
		(((CASE WHEN (A.STATUS IN ('fechado', 'fechado_sem_resposta') AND A.FILA_ATENDIMENTO = 'suporte_empresas' AND ISNULL(A.DATA_FECHAMENTO, A.DATA_MODIFICACAO) < A.DATA_INCLUSAO_NORMALIZADA) THEN 0 ELSE C.DIFERENCA_HORAS END)*60*60)/60/60/24) AS TMS
	INTO	#TMR_CASOS
