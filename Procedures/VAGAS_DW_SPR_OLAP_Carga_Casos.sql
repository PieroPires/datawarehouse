...
SELECT ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,STATUS,PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,
	   ...
	   CASE WHEN DATEPART(WEEKDAY,DATA_INCLUSAO_NORMALIZADA) = 7 -- SÁBADO
			...
			ELSE DATA_INCLUSAO_NORMALIZADA END AS DATA_INCLUSAO_NORMALIZADA, CONTA_ID, UPPER(IIF(MERCADO IS NULL OR MERCADO = '', 'EM BRANCO', MERCADO)) AS MERCADO,TIPO_CLIENTE,MOTIVO_CONTATO
...
FROM 
	...
	SELECT ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,STATUS,PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,
			...
			CASE WHEN (DATEPART(HOUR,DATA_INCLUSAO) >= 19 AND DATEPART(MINUTE, DATA_INCLUSAO) > 0) OR (DATEPART(HOUR, DATA_INCLUSAO) > 19)
							...
							ELSE DATA_INCLUSAO END AS DATA_INCLUSAO_NORMALIZADA, CONTA_ID, MERCADO, EMAIL_CONTATO,TIPO_CLIENTE,MOTIVO_CONTATO
...
-- Inserir novos casos 
INSERT INTO VAGAS_DW.CASOS (ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,[STATUS],PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,DATA_FECHAMENTO,  DATA_INCLUSAO,DATA_MODIFICACAO,USUARIO_RESPONSAVEL,ORIGEM_SUPORTE,DATA_PRM_ACAO,DATA_INCLUSAO_NORMALIZADA,TME, TMS, CONTA_ID, MERCADO, EMAIL_CONTATO,TIPO_CLIENTE,MOTIVO_CONTATO) 
SELECT  ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,[STATUS],PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,DATA_FECHAMENTO,  DATA_INCLUSAO,DATA_MODIFICACAO,USUARIO_RESPONSAVEL,ORIGEM_SUPORTE,DATA_PRM_ACAO,DATA_INCLUSAO_NORMALIZADA,TME, TMS, CONTA_ID, MERCADO, EMAIL_CONTATO,TIPO_CLIENTE,MOTIVO_CONTATO 
FROM #TMR_CASOS
...
-- Inclusão do QTD_TOTAL_CASO_EMAIL_2H:
UPDATE	[VAGAS_DW].[CASOS]
SET		[QTD_TOTAL_CASO_EMAIL_2H] = (SELECT	IIF(TME * 24 <= 2, 1, 0))
