...
WHERE EXISTS ( SELECT 1 FROM STAGE.VAGAS_DW.TMP_CASOS
...
SELECT ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,STATUS,PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,
		    DATA_FECHAMENTO,DATA_INCLUSAO,DATA_MODIFICACAO,USUARIO_RESPONSAVEL,ORIGEM_SUPORTE,DATA_PRM_ACAO,EMAIL_CONTATO,
	   CASE WHEN DATEPART(WEEKDAY,DATA_INCLUSAO_NORMALIZADA) = 7 -- SÁBADO
			...
			ELSE DATA_INCLUSAO_NORMALIZADA END AS DATA_INCLUSAO_NORMALIZADA, CONTA_ID, UPPER(IIF(MERCADO IS NULL OR MERCADO = '', 'EM BRANCO', MERCADO)) AS MERCADO,TIPO_CLIENTE,MOTIVO_CONTATO,NOME_CONTATO_EMAIL,CONTATO_ADM_PRINCIPAL,SEGMENTO_SC
...
FROM 
	...
	SELECT ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,STATUS,PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,
			...
			CASE WHEN (DATEPART(HOUR,DATA_INCLUSAO) >= 19 AND DATEPART(MINUTE, DATA_INCLUSAO) > 0) OR (DATEPART(HOUR, DATA_INCLUSAO) > 19)
							...
							ELSE DATA_INCLUSAO END AS DATA_INCLUSAO_NORMALIZADA, CONTA_ID, MERCADO, EMAIL_CONTATO,TIPO_CLIENTE,MOTIVO_CONTATO,NOME_CONTATO_EMAIL,CONTATO_ADM_PRINCIPAL,SEGMENTO_SC
	FROM STAGE.VAGAS_DW.TMP_CASOS
...
SELECT	A.NUMERO_CASO ,
		...
		A.DATA_FECHAMENTO AS DATA_FECHAMENTO ,
		A.DATA_PRM_ACAO ,
...
-- DROP TABLE #TMP_CONTROLE_REGISTROS_TME ;
SELECT	A.NUMERO_CASO ,
		COUNT(*) AS QTD_REGISTROS
INTO	#TMP_CONTROLE_REGISTROS_TME
...
SELECT	A.NUMERO_CASO ,
		...
		DATA_PRM_ACAO ,
		DATA_HORA ,
		IIF(CONVERT(DATE, DATA_INCLUSAO_NORMALIZADA) = CONVERT(DATE, DATA_PRM_ACAO) AND DATEPART(HOUR, DATA_INCLUSAO_NORMALIZADA) = 8, DATA_INCLUSAO_NORMALIZADA, MENOR_DATA_HORA) AS MENOR_DATA_HORA ,
		IIF(QTD_REGISTROS = 1, DATA_PRM_ACAO, MAIOR_DATA_HORA) AS MAIOR_DATA_HORA ,
		HORA_UTIL
...
FROM	#HORA_UTIL AS A			CROSS APPLY (SELECT	TOP 1 A1.DATA_HORA AS MAIOR_DATA_HORA
										 FROM	#HORA_UTIL AS A1
										 WHERE	A.NUMERO_CASO = A1.NUMERO_CASO
												AND CONVERT(DATE, A.DATA_HORA) = CONVERT(DATE, A1.DATA_HORA)
												AND A1.HORA_UTIL = 1
												ORDER BY A1.DATA_HORA DESC) AS C
								CROSS APPLY (SELECT	TOP 1 IIF(DATEPART(HOUR, A1.DATA_HORA) = 8 
														  AND DATEPART(HOUR, C.MAIOR_DATA_HORA) = 8, DATEADD(HOUR, 8, CONVERT(SMALLDATETIME, CONVERT(DATE, A1.DATA_HORA))), A1.DATA_HORA) AS MENOR_DATA_HORA
										 FROM	#HORA_UTIL AS A1
										 WHERE	A.NUMERO_CASO = A1.NUMERO_CASO
												AND CONVERT(DATE, A.DATA_HORA) = CONVERT(DATE, A1.DATA_HORA)
												AND A1.HORA_UTIL = 1
												ORDER BY A1.DATA_HORA ASC) AS B
							  LEFT OUTER JOIN #TMP_CONTROLE_REGISTROS_TME AS D ON A.NUMERO_CASO = D.NUMERO_CASO
WHERE	A.HORA_UTIL = 1 ;
...
SELECT	A.NUMERO_CASO ,
		...
		CASE WHEN (CONVERT(DATE, A.DATA_INCLUSAO_NORMALIZADA) = CONVERT(DATE, B.DATA_HORA) AND DATEPART(HOUR, A.DATA_INCLUSAO_NORMALIZADA) = DATEPART(HOUR, B.DATA_HORA)) 
				THEN A.DATA_INCLUSAO_NORMALIZADA
			 WHEN (CONVERT(DATE, A.DATA_FECHAMENTO) = CONVERT(DATE, B.DATA_HORA) AND DATEPART(HOUR, A.DATA_FECHAMENTO) = DATEPART(HOUR, B.DATA_HORA)) 
				THEN A.DATA_FECHAMENTO
			 ELSE B.DATA_HORA
		END AS DATA_HORA
INTO	#FORMAT_DATA_HORA_TMS
FROM	#TMP_DATA_NORMALIZADA AS A	LEFT OUTER JOIN #TMP_DATA_HORA AS B ON B.DATA_HORA >= DATEADD(MINUTE, DATEPART(MINUTE, A.DATA_INCLUSAO_NORMALIZADA) * -1, A.DATA_INCLUSAO_NORMALIZADA) AND B.DATA_HORA <= A.DATA_FECHAMENTO
WHERE	STATUS IN ('fechado', 'fechado_sem_resposta') AND FILA_ATENDIMENTO = 'suporte_empresas'
ORDER BY DATA_HORA ASC ;


-- DROP TABLE #TMP_CONTROLE_REGISTROS_TMS ;
SELECT	A.NUMERO_CASO ,
		COUNT(*) AS QTD_REGISTROS
INTO	#TMP_CONTROLE_REGISTROS_TMS
FROM	#FORMAT_DATA_HORA_TMS AS A
GROUP BY
		A.NUMERO_CASO ;
...
SELECT	A.NUMERO_CASO ,
		...
		DATA_FECHAMENTO ,
		DATA_HORA ,
		IIF(CONVERT(DATE, DATA_INCLUSAO_NORMALIZADA) = CONVERT(DATE, DATA_FECHAMENTO) AND DATEPART(HOUR, DATA_INCLUSAO_NORMALIZADA) = 8, DATA_INCLUSAO_NORMALIZADA, MENOR_DATA_HORA) AS MENOR_DATA_HORA ,
		IIF(QTD_REGISTROS = 1, DATA_FECHAMENTO, MAIOR_DATA_HORA) AS MAIOR_DATA_HORA ,
		HORA_UTIL
...
--												AND A1.HORA_UTIL = 1
--												ORDER BY A1.DATA_HORA DESC) AS C
FROM	#HORA_UTIL_TMS AS A		CROSS APPLY (SELECT	TOP 1 A1.DATA_HORA AS MAIOR_DATA_HORA
										 FROM	#HORA_UTIL_TMS AS A1
										 WHERE	A.NUMERO_CASO = A1.NUMERO_CASO
												AND CONVERT(DATE, A.DATA_HORA) = CONVERT(DATE, A1.DATA_HORA)
												AND A1.HORA_UTIL = 1
								...
								CROSS APPLY (SELECT	TOP 1 IIF(DATEPART(HOUR, A1.DATA_HORA) = 8 
										 ...
										 WHERE	A.NUMERO_CASO = A1.NUMERO_CASO
												...
												AND A1.HORA_UTIL = 1
												ORDER BY A1.DATA_HORA ASC) AS B
							  LEFT OUTER JOIN #TMP_CONTROLE_REGISTROS_TMS AS D ON A.NUMERO_CASO = D.NUMERO_CASO
WHERE	A.HORA_UTIL = 1 ;
...
GROUP BY
	...
	SELECT ID_CASO ,
		...
		(CASE WHEN (A.STATUS IN ('fechado', 'fechado_sem_resposta') AND A.FILA_ATENDIMENTO = 'suporte_empresas' AND A.DATA_FECHAMENTO < A.DATA_INCLUSAO_NORMALIZADA) THEN 0 ELSE C.DIFERENCA_HORAS END) / 24 AS TMS ,
		CONTA_ID ,
		MERCADO ,
		A.EMAIL_CONTATO ,
		A.TIPO_CLIENTE,
...
-- Inserir novos casos 
INSERT INTO VAGAS_DW.CASOS (ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,[STATUS],PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,DATA_FECHAMENTO,  DATA_INCLUSAO,DATA_MODIFICACAO,USUARIO_RESPONSAVEL,ORIGEM_SUPORTE,DATA_PRM_ACAO,DATA_INCLUSAO_NORMALIZADA,TME, TMS, CONTA_ID, MERCADO, EMAIL_CONTATO,TIPO_CLIENTE,MOTIVO_CONTATO,NOME_CONTATO_EMAIL,CONTATO_ADM_PRINCIPAL,SEGMENTO_SC) 
SELECT  ID_CASO,NUMERO_CASO,CONTA,FILA_ATENDIMENTO,[STATUS],PRIORIDADE,CRITICIDADE,ORIGEM,TIPO,SUB_TIPO,DATA_INICIO_ATENDIMENTO,DATA_FECHAMENTO,  DATA_INCLUSAO,DATA_MODIFICACAO,USUARIO_RESPONSAVEL,ORIGEM_SUPORTE,DATA_PRM_ACAO,DATA_INCLUSAO_NORMALIZADA,TME, TMS, CONTA_ID, MERCADO, EMAIL_CONTATO,TIPO_CLIENTE,MOTIVO_CONTATO,NOME_CONTATO_EMAIL,CONTATO_ADM_PRINCIPAL,SEGMENTO_SC
FROM #TMR_CASOS
...
-- Inclusão do QTD_TOTAL_CASO_EMAIL_4H:
UPDATE	[VAGAS_DW].[CASOS]
SET		[QTD_TOTAL_CASO_EMAIL_4H] = (SELECT	IIF(TME * 24 <= 4, 1, 0))
FROM	[VAGAS_DW].[CASOS] AS A
WHERE	A.ORIGEM_SUPORTE = 'e-mail' ;

-- Cálculo da MEDIA_CASOS_ANO_USUARIO:
UPDATE	[VAGAS_DW].[CASOS]
SET		MEDIA_CASOS_ANO_USUARIO = (SELECT	CONVERT(FLOAT, QTD_CASOS_ANO)/QTD_ANOS_UNICOS AS MEDIA_CASOS_ANO_USUARIO
								   FROM	(SELECT	*,	
											(SELECT	COUNT(*) AS QTD_CASOS
											 FROM	[VAGAS_DW].[CASOS] AS A1
											 WHERE	A.USUARIO_RESPONSAVEL = A1.USUARIO_RESPONSAVEL
													AND A1.FILA_ATENDIMENTO = 'suporte_empresas'
													AND A1.[STATUS] = 'fechado'
													AND ISNULL(A1.USUARIO_RESPONSAVEL, '') != ''
													AND ISNULL(A1.DATA_FECHAMENTO, '') != '') AS QTD_CASOS_ANO ,
											(SELECT	COUNT(DISTINCT DATEPART(YEAR, A1.DATA_FECHAMENTO)) AS QTD_ANOS_UNICOS
											 FROM	[VAGAS_DW].[CASOS] AS A1
											 WHERE	A.USUARIO_RESPONSAVEL = A1.USUARIO_RESPONSAVEL
													AND A1.FILA_ATENDIMENTO = 'suporte_empresas'
													AND A1.[STATUS] = 'fechado'
													AND ISNULL(A1.USUARIO_RESPONSAVEL, '') != ''
													AND ISNULL(A1.DATA_FECHAMENTO, '') != '') AS QTD_ANOS_UNICOS
										FROM	[VAGAS_DW].[CASOS] AS A
										WHERE	A.FILA_ATENDIMENTO = 'suporte_empresas'
												AND A.[STATUS] = 'fechado'
												AND ISNULL(A.USUARIO_RESPONSAVEL, '') != ''
												AND ISNULL(A.DATA_FECHAMENTO, '') != ''
												AND A.ID_CASO = B.ID_CASO) AS SUBQUERY)
FROM	[VAGAS_DW].[CASOS] AS B
WHERE	B.FILA_ATENDIMENTO = 'suporte_empresas'
		AND B.[STATUS] = 'fechado'
		AND ISNULL(B.USUARIO_RESPONSAVEL, '') != ''
		AND ISNULL(B.DATA_FECHAMENTO, '') != '' ;


-- Cálculo da MEDIA_CASOS_MESES_ANO_USUARIO:
UPDATE	[VAGAS_DW].[CASOS]
SET		MEDIA_CASOS_MESES_ANO_USUARIO = (SELECT	CONVERT(FLOAT, QTD_CASOS_MES)/QTD_MESES_UNICOS AS MEDIA_CASOS_MESES_ANO_USUARIO
										FROM	(SELECT	*,	
													(SELECT	COUNT(*) AS QTD_CASOS
													 FROM	[VAGAS_DW].[CASOS] AS A1
													 WHERE	DATEPART(YEAR, A.DATA_FECHAMENTO) = DATEPART(YEAR, A1.DATA_FECHAMENTO)
															AND A.USUARIO_RESPONSAVEL = A1.USUARIO_RESPONSAVEL
															AND A1.FILA_ATENDIMENTO = 'suporte_empresas'
															AND A1.[STATUS] = 'fechado'
															AND ISNULL(A1.USUARIO_RESPONSAVEL, '') != ''
															AND ISNULL(A1.DATA_FECHAMENTO, '') != '') AS QTD_CASOS_MES ,
													(SELECT	COUNT(DISTINCT DATEPART(MONTH, A1.DATA_FECHAMENTO)) AS QTD_MESES_UNICOS
													 FROM	[VAGAS_DW].[CASOS] AS A1
													 WHERE	DATEPART(YEAR, A.DATA_FECHAMENTO) = DATEPART(YEAR, A1.DATA_FECHAMENTO)
															AND A.USUARIO_RESPONSAVEL = A1.USUARIO_RESPONSAVEL
															AND A1.FILA_ATENDIMENTO = 'suporte_empresas'
															AND A1.[STATUS] = 'fechado'
															AND ISNULL(A1.USUARIO_RESPONSAVEL, '') != ''
															AND ISNULL(A1.DATA_FECHAMENTO, '') != '') AS QTD_MESES_UNICOS
										FROM	[VAGAS_DW].[CASOS] AS A
										WHERE	A.FILA_ATENDIMENTO = 'suporte_empresas'
												AND A.[STATUS] = 'fechado'
												AND ISNULL(A.USUARIO_RESPONSAVEL, '') != ''
...
-- Cálculo da MEDIA_CASOS_DIAS_MESES_ANO_USUARIO:
UPDATE	[VAGAS_DW].[CASOS]
SET		MEDIA_CASOS_DIAS_MESES_ANO_USUARIO = (SELECT	CONVERT(FLOAT, QTD_CASOS_DIA_MES)/QTD_DIAS_UNICOS AS MEDIA_CASOS_DIAS_MESES_ANO_USUARIO
											   FROM	(SELECT	*,	
														(SELECT	COUNT(*) AS QTD_CASOS_DIA_MES
														 FROM	[VAGAS_DW].[CASOS] AS A1
														 WHERE	DATEPART(YEAR, A.DATA_FECHAMENTO) = DATEPART(YEAR, A1.DATA_FECHAMENTO)
																AND DATEPART(MONTH, A.DATA_FECHAMENTO) = DATEPART(MONTH, A1.DATA_FECHAMENTO)
																AND A.USUARIO_RESPONSAVEL = A1.USUARIO_RESPONSAVEL
																AND A1.FILA_ATENDIMENTO = 'suporte_empresas'
																AND A1.[STATUS] = 'fechado'
																AND ISNULL(A1.USUARIO_RESPONSAVEL, '') != ''
																AND ISNULL(A1.DATA_FECHAMENTO, '') != '') AS QTD_CASOS_DIA_MES ,
														(SELECT	COUNT(DISTINCT DATEPART(DAY, A1.DATA_FECHAMENTO)) AS QTD_DIAS_UNICOS
														 FROM	[VAGAS_DW].[CASOS] AS A1
														 WHERE	DATEPART(YEAR, A.DATA_FECHAMENTO) = DATEPART(YEAR, A1.DATA_FECHAMENTO)
																AND DATEPART(MONTH, A.DATA_FECHAMENTO) = DATEPART(MONTH, A1.DATA_FECHAMENTO)
																AND A.USUARIO_RESPONSAVEL = A1.USUARIO_RESPONSAVEL
																...
																AND A1.[STATUS] = 'fechado'
																AND ISNULL(A1.USUARIO_RESPONSAVEL, '') != ''
																AND ISNULL(A1.DATA_FECHAMENTO, '') != '') AS QTD_DIAS_UNICOS
										FROM	[VAGAS_DW].[CASOS] AS A
										WHERE	A.FILA_ATENDIMENTO = 'suporte_empresas'
												AND A.[STATUS] = 'fechado'
												AND ISNULL(A.USUARIO_RESPONSAVEL, '') != ''
...
-- 07/08/2018: Aplicação de uma nova regra que foi acordada com a equipe de Suporte a Empresas.
-- Regra: Quando o status do caso for igual a FECHADO, e a DATA_FECHAMENTO for em branco, atribuir 30 minutos ao campo TMS.
UPDATE	[VAGAS_DW].[CASOS]
SET		TMS = 0.020833333
FROM	[VAGAS_DW].[CASOS] AS A
...
-- 10/08/2018: Aplicação de uma nova regra: Quando o TME for > que o TMS, atribuir o TMS ao TME.
UPDATE	[VAGAS_DW].[CASOS]
SET		TME = A.TMS
FROM	[VAGAS_DW].[CASOS] AS A
WHERE	A.TME > A.TMS
