...
FROM	[VAGAS_DW].[TMP_CLIENTES] AS A		LEFT OUTER JOIN [VAGAS_DW].[CONTAS_CRM] AS B ON A.CONTA_ID = B.CONTA_ID
...
UPDATE VAGAS_DW.TMP_CLIENTES SET CONTEM_RECRUTADOR = 1,
...
UPDATE VAGAS_DW.TMP_CLIENTES SET --CLIENTE_CRM = B.CONTA,
								 PERFIL_CRM = B.PERFIL,
								 SEGMENTO = B.NEGOCIO,
								 CNPJ = B.CNPJ
FROM VAGAS_DW.TMP_CLIENTES A
...
--					A1.CONTA_CRM , 
--					A1.ID_VAGAS 
--		  FROM		[VAGAS_DW].[CONTAS_MEMBRO_CRM] AS A1 ) AS SUBQUERY		INNER JOIN [VAGAS_DW].[TMP_CLIENTES] AS B ON SUBQUERY.ID_VAGAS = B.CLIENTE_VAGAS				   
--WHERE	EXISTS ( SELECT	1 
--				 FROM	[VAGAS_DW].[TMP_CLIENTES] AS A1 
--				 WHERE	SUBQUERY.CONTA_ID = A1.CONTA_ID 
--						AND A1.CONTA_ID IN ( SELECT	AA1.CONTA_ID 
--											 FROM	[VAGAS_DW].[TMP_CLIENTES] AS AA1 
--											 GROUP BY 
--													AA1.CONTA_ID 
--											 HAVING COUNT(*) > 1) ) 
...
SET	@PRM_COD_HISTCAND = (	SELECT	MIN(A1.Cod_HistCand)
							FROM	[VAGAS_DW].[VAGAS_DW].[CANDIDATURAS] AS A1
