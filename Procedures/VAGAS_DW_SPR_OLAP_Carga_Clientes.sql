...
WHERE CONTA_ID IN ( SELECT CONTA_ID FROM VAGAS_DW.CLIENTES GROUP BY CONTA_ID HAVING COUNT(*) > 1 )
AND EX_CLIENTE = 1
-- MANTER APENAS ÚLTIMO PERFIL CADASTRADO
DELETE VAGAS_DW.CLIENTES 
FROM VAGAS_DW.CLIENTES A
WHERE CONTA_ID IN ( SELECT CONTA_ID FROM VAGAS_DW.CLIENTES GROUP BY CONTA_ID HAVING COUNT(*) > 1 )
AND DATA_CONTRATO_MANUT <> ( SELECT MAX(DATA_CONTRATO_MANUT) FROM VAGAS_DW.CLIENTES WHERE CONTA_ID = A.CONTA_ID )
