...
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES]

-- Mantém apenas registros onde o ID_VAGAS exista nas visões de CONTAS do CRM:
DELETE FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES]
FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A
WHERE	A.CONTA_ID IN (SELECT	A1.CONTA_ID
						FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A1
		...
		AND NOT (EXISTS (SELECT	*
					 ...
					 WHERE	A.CONTA_ID = A1.CONTA_ID
							AND A.CLIENTE_VAGAS COLLATE Latin1_General_CS_AS  = A1.ID_VAGAS COLLATE Latin1_General_CS_AS)
			OR EXISTS (SELECT *
...
-- Atualiza campos provindos do CRM:
UPDATE	[STAGE].[VAGAS_DW].[TMP_CLIENTES]
SET		 CLIENTE_CRM = ISNULL(B.CONTA_CRM, C.CONTA_CRM)
		,CNPJ = COALESCE(NULLIF(C.CNPJ,''), NULLIF(B.CNPJ,''), NULLIF(A.CNPJ,''))
FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A		LEFT OUTER JOIN [VAGAS_DW].[CONTAS_CRM] AS B ON A.CONTA_ID = B.CONTA_ID
											LEFT OUTER JOIN [VAGAS_DW].[CONTAS_MEMBRO_CRM] AS C ON A.CONTA_ID = C.CONTA_ID ;

/*
############################################################
#
################## CASOS GERAIS DE CLIENTE #################
#
############################################################
*/

-- MARCAR CLIENTES COM CONTRATOS NO ERP COMO ATIVOS
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 0 
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA ERP - 0', REGRA_EX_CLIENTE + ' | REGRA ERP - 0')
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A
	JOIN STAGE.VAGAS_DW.TMP_CONTRATOS_ERP AS B ON A.CONTA_ID = B.CONTA_ID
WHERE B.SITUACAO = 'ATIVO'
	AND B.SEQUENCIA = 1

-- MARCAR CLIENTES ATIVOS DO MANUT
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 0 
...
-- Marcar clientes em aviso prévio como clientes ativos
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES]
SET EX_CLIENTE = 0
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA AVISO PREVIO - 0', REGRA_EX_CLIENTE + ' | REGRA AVISO PREVIO - 0')
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE  EXISTS ( SELECT 1 FROM VAGAS_DW.CONTAS_CRM WHERE CONTA_ID = A.CONTA_ID AND (TIPO = 'cliente_em_aviso_previo' OR CATEGORIA = 'cliente_em_aviso_previo')
				UNION
				SELECT 1 FROM VAGAS_DW.CONTAS_MEMBRO_CRM WHERE CONTA_ID = A.CONTA_ID AND (TIPO = 'cliente_em_aviso_previo' OR CATEGORIA = 'cliente_em_aviso_previo')
				)
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] 
SET QTD_VAGAS_PUBLICADAS = TAB.QTD_VAGAS_PUBLICADAS
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 0,
								REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA FLEXC - 0', REGRA_EX_CLIENTE + ' | REGRA FLEXC - 0'),
								 TIPO_CLIENTE = 'FLEX_C' ,
								 CONTEM_FLEX_C = 1
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE TIPO_CLIENTE_MANUT = 'FLEXC' 
AND COALESCE(TIPO_CLIENTE,'') = ''
AND ISNULL(CONTEM_FLEX_C,0) = 0 
AND ( QTD_CREDITOS_DISPONIVEL > 0 OR QTD_VAGAS_ULT_6_MESES > 0 )

UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 0,
								REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA FLEXA - 0', REGRA_EX_CLIENTE + ' | REGRA FLEXA - 0'),
								 TIPO_CLIENTE = 'FLEX_A' ,
								 CONTEM_FLEX_A = 1
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
...
-- Os que sobraram são marcados como ex-cliente
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 1
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA NULL - 1', REGRA_EX_CLIENTE + ' | REGRA NULL - 1')
WHERE EX_CLIENTE IS NULL
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_FIT = 1,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_REDES = 1,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_SMS = 1,
...
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL, CNPJ
			  FROM VAGAS_DW.OPORTUNIDADES 
			  WHERE CONTAID  = A.CONTA_ID
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FECHADO_GANHO = 1
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,CNPJ
			    ) B
--WHERE A.EX_CLIENTE =  0 

-- MARCAR CLIENTES COM PRODUTO PET ATIVO
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_PET = 1,
...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL,CNPJ
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FECHADO_GANHO = 1
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,CONTA,PERFIL,NEGOCIO,CNPJ
			    ) B
--WHERE A.EX_CLIENTE =  0 

-- MARCAR CLIENTES COM PRODUTO RECRUTADOR ATIVO
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_RECRUTADOR = 1,
								 VALOR_RECRUTADOR = B.ValorProdutoFINAL,
								 --CLIENTE_CRM = B.CONTA,
								 PERFIL_CRM = B.PERFIL,
								 SEGMENTO = B.NEGOCIO,
								 CNPJ = B.CNPJ,
...
CROSS APPLY ( SELECT TOP 1 * 
				FROM VAGAS_DW.OPORTUNIDADES
			   WHERE CONTAID  = A.CONTA_ID
			   AND PRODUTO_GRUPO IN ('RECRUTADOR')
			   AND FECHADO_GANHO = 1
			   AND RECORRENTE = 1
			   ORDER BY DataFechamento DESC ) B
WHERE B.OPORTUNIDADECATEGORIA <> 'rescisao'
...
WHERE
	--A.EX_CLIENTE =  0 
	A.TIPO_CLIENTE_MANUT = 'FLEXC'

-- MARCAR CLIENTES COM PRODUTO FLEXA
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_FLEX_A = 1,
...
-- ATUALIZAR PRC
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_PRC = 1,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 1,
								REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA FLEX A/C - 1', REGRA_EX_CLIENTE + ' | REGRA FLEX A/C - 1'),
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 1,
								REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA SEM CREDITO/VAGA ULT 6 MESES - 1', REGRA_EX_CLIENTE + ' | REGRA SEM CREDITO/VAGA ULT 6 MESES - 1'),
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 1,
								REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA SEM PRODUTOS - 1', REGRA_EX_CLIENTE + ' | REGRA SEM PRODUTOS - 1'),
								 TIPO_CLIENTE_MANUT = 'FIRST',
...
SET EX_CLIENTE = 1
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA FONTE CRM - 1', REGRA_EX_CLIENTE + ' | REGRA FONTE CRM - 1')
FROM [VAGAS_DW].[CLIENTES] AS A
WHERE A.FONTE = 'CRM'

-- MARCAR EX_CLIENTES QUE ESTEJAM MARCADOS COMO "EX_CLIENTES" NO CRM
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 1
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA EX-CLIENTE CRM - 1', REGRA_EX_CLIENTE + ' | REGRA EX-CLIENTE CRM - 1')
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE  EXISTS ( SELECT * 
		         FROM VAGAS_DW.OPORTUNIDADES 
		         WHERE CONTAID  = A.CONTA_ID 
...
-- MARCAR COMO EX_CLIENTES AQUELES QUE NÃO CONSTAM COM OPORTUNIDADES (FECHADAS E GANHAS) NO CRM E NÃO SEJAM CLIENTE DO TIPO CRÉDITOS VAGAS
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET EX_CLIENTE = 1
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA NO OPP FECHADO_GANHO - 1', REGRA_EX_CLIENTE + ' | REGRA NO OPP FECHADO_GANHO - 1')
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
...
-- Marcar como EX_CLIENTES, contas do CRM, que estão com o TIPO e a CATEGORIA iguais a cliente_potencial
UPDATE	[STAGE].[VAGAS_DW].[TMP_CLIENTES]
SET		EX_CLIENTE = 1
	,REGRA_EX_CLIENTE = IIF(REGRA_EX_CLIENTE IS NULL, 'REGRA CLIENTE POTENCIAL - 1', REGRA_EX_CLIENTE + ' | REGRA CLIENTE POTENCIAL - 1')
FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A
WHERE	EXISTS (SELECT	*
					FROM	( 
								SELECT	A1.CONTA_ID
...
--UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET VALOR_MENSALIDADE = ISNULL(VALOR_FIT,0) + ISNULL(VALOR_REDES,0) 
...
CREATE NONCLUSTERED INDEX IDX_#TMP_VALOR_PRINCIPAL ON #TMP_VALOR_PRINCIPAL (CONTA_ID);

-- Atualização do valor da mensalidade pra clientes que possuem o VAGAS FIT ANUID:
SELECT	A.CONTA_ID ,
		CASE PRODUTO_RECORRENCIA
			WHEN 'annually' THEN A.VALOR_RECORRENTE / 12 
			WHEN 'monthly' THEN A.VALOR_RECORRENTE
			ELSE NULL
...
CREATE NONCLUSTERED INDEX IDX_#TMP_VALOR_MENSALIDADE_FIT_ANUID_CONTAID ON #TMP_VALOR_MENSALIDADE_FIT_ANUID (CONTA_ID);
...
CREATE NONCLUSTERED INDEX IDX_#TMP_VALOR_MENSALIDADE_FIT ON #TMP_VALOR_MENSALIDADE_FIT (CONTA_ID);


UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET VALOR_MENSALIDADE = B.VALOR_MENSALIDADE ,
								 TIPO_CLIENTE = CASE WHEN CONTEM_FIT = 1 THEN 'FIT' ELSE '' END
											  ...
											  + CASE WHEN CONTEM_RECRUTADOR = 1 THEN ' | RECRUTADOR' ELSE '' END
...
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A		LEFT OUTER JOIN (SELECT		A.CONTA_ID ,
																A.VALOR_MENSALIDADE
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET --CLIENTE_CRM = B.CONTA,
								 PERFIL_CRM = B.PERFIL,
								 SEGMENTO = B.NEGOCIO,
								 CNPJ = B.CNPJ
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET DATA_CONTRATO_CRM = B.FIM_VIGENCIA,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET DATA_RESCISAO = DATAFECHAMENTO,
								 TIPO_CLIENTE_MANUT = 'FIRST'
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET FLAG_ATIVO_SEM_VAGAS = 1
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE A.QTD_Vagas_ULT_6_Meses = 0 AND A.EX_CLIENTE = 0

-- marcar data da primeira oportunidade no CRM
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET DATA_PRM_OPORTUNIDADE_CRM = B.DATAFECHAMENTO
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
CROSS APPLY ( SELECT TOP 1 * 
			  FROM VAGAS_DW.OPORTUNIDADES 
			  WHERE CONTAID = A.CONTA_ID
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET FLAG_CLIENTE_NOVO = 1
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE A.EX_CLIENTE = 0 AND DATA_PRM_OPORTUNIDADE_CRM >= DATEADD(MONTH,-12,GETDATE())

-- Atualizar Médias 
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_Vagas_ULT_3_Meses = CONVERT(FLOAT,A.QTD_Vagas_ULT_3_Meses) / 3 ,
								 ...
								 SALDO_BCE = CONVERT(FLOAT, QTD_BCE) / IIF(QTD_BCE > 0 AND CAPACIDADE_BCE = 0, CONVERT(FLOAT, 1), CONVERT(FLOAT, CAPACIDADE_BCE))
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE EX_CLIENTE = 0
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_VAGAS_PONDERADO = CASE WHEN DATEDIFF(MONTH,DATA_REFERENCIA_12_MESES_INICIO,DATA_PRM_OPORTUNIDADE_CRM) + 1 = 12 
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_VAGAS_PONDERADO = MEDIA_Vagas_ULT_12_Meses
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES]
WHERE EX_CLIENTE = 0
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_Vagas_PROJECAO = CASE WHEN DATEDIFF(MONTH,DATA_REFERENCIA_12_MESES_INICIO,DATA_PRM_OPORTUNIDADE_CRM) + 1 = 12 THEN CONVERT(FLOAT,QTD_VAGAS_MES_12) / PESO_MES_12
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_VAGAS_PROJECAO = 1
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_VAGAS_PONDERADO = ISNULL(TAB.MEDIA_PONDERADA,0),
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET DATA_PRM_VAGA_PUBLICADA = TAB.DATA_PRM_VAGA_PUBLICADA,
							 DATA_ULT_VAGA_PUBLICADA = TAB.DATA_ULT_VAGA_PUBLICADA,
...
/*
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET DIAS_ATRASO_ULT_FATURA = B.DIAS_ATRASO,
...
SELECT
	...
	,FLAG_PAGO
	,ROW_NUMBER() OVER (PARTITION BY COD_CLI_CRM ORDER BY DATA_VENCIMENTO_REAL_ORIGINAL DESC) AS NUM_LINHA
INTO #TMP_FATURAS_VENCIDAS
FROM VAGAS_DW.FATURAS
WHERE TIPO_ORIGEM = 'NF'
	AND DATA_REFERENCIA = ( SELECT MAX(DATA_REFERENCIA) fROM VAGAS_DW.FATURAS )
	AND DATA_VENCIMENTO_REAL_ORIGINAL <= GETDATE() -- ÚLT. FATURA VENCIDA

DELETE FROM  #TMP_FATURAS_VENCIDAS WHERE NUM_LINHA > 1

CREATE NONCLUSTERED INDEX IDX_#TMP_FATURAS_VENCIDAS_CONTA_ID ON #TMP_FATURAS_VENCIDAS (COD_CLI_CRM)
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET DIAS_ATRASO_ULT_FATURA = B.DIAS_ATRASO,
								 ...
								 FLAG_PAGO = B.FLAG_PAGO
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A
	JOIN #TMP_FATURAS_VENCIDAS AS B ON COD_CLI_CRM = A.CONTA_ID
WHERE A.EX_CLIENTE = 0

UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTEM_CUSTOMIZACAO = 1,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET FLAG_CLIENTE_NOVO = CASE WHEN FLAG_CLIENTE_NOVO IS NULL THEN 0 ELSE 1 END,
...
INNER JOIN [STAGE].[VAGAS_DW].[TMP_CLIENTES] B ON B.COD_CLI = A.COD_CLI
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET QTD_EXTRACOES_BCC = ISNULL(B.QTD_CVS,0)
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
LEFT OUTER JOIN #TMP_EXTRACOES_BCC B ON B.COD_CLI = A.COD_CLI
									 AND B.TIPO_BCC = '12_MESES'
WHERE A.EX_CLIENTE = 0

UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET QTD_EXTRACOES_BCC_6_MESES = ISNULL(B.QTD_CVS,0) ,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET QTD_EXTRACOES_BCC_3_MESES = ISNULL(B.QTD_CVS,0)
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
LEFT OUTER JOIN #TMP_EXTRACOES_BCC B ON B.COD_CLI = A.COD_CLI
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET MEDIA_VAGAS_ULT_6_MESES_RE = ISNULL(CONVERT(FLOAT,B.QTD_VAGAS_RE) / 6,0)  , -- CONSIDERAMOS TAMBÉM A EQUIV. DO BCC NA MÉDIA DO RE
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET STATUS_BCE = CASE WHEN SALDO_BCE < 0.95 THEN 'ABAIXO'
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET STATUS_CLIENTE = CASE WHEN (STATUS_VAGAS_RE = 'ACIMA' OR STATUS_VAGAS_RI = 'ACIMA') AND STATUS_BCE IN ('REVISÃO','ACIMA') 
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET STATUS_POSICOES_REDES = CASE WHEN MEDIA_POSICOES_ULT_6_MESES_REDES > POSICOES_MES THEN 'ACIMA'
														  ELSE 'ABAIXO' END,
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET STATUS_REDES = CASE WHEN (    ( STATUS_POSICOES_REDES = 'ACIMA' AND STATUS_UNIDADES_REDES = 'ABAIXO' )
...
--UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET TEMPO_INATIVIDADE = CASE WHEN DATA_ULT_VAGA_PUBLICADA IS NULL THEN 'NUNCA PUBLICOU'
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET TEMPO_INATIVIDADE = 'Cliente Bloqueado'
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE A.EX_CLIENTE = 0
AND CONTEM_FIT = 1
AND EXISTS ( SELECT * 
...
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES]  SET CONTA_ID = B.CONTA_ID
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES]  A
...
AND NOT EXISTS ( SELECT * 
				 FROM VAGAS_DW.OPORTUNIDADES
				 WHERE ID_VAGAS COLLATE Latin1_General_CS_AS  = A.CLIENTE_VAGAS COLLATE Latin1_General_CS_AS ) 

-- CLIENTES SEM CONTA COM ALGUMA OPORTUNIDADE EXISTENTE
UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CONTA_ID = B.CONTAID
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
CROSS APPLY ( SELECT TOP 1 CONTAID 
			  FROM VAGAS_DW.OPORTUNIDADES
			  WHERE ID_VAGAS COLLATE Latin1_General_CS_AS  = A.CLIENTE_VAGAS COLLATE Latin1_General_CS_AS ) B
WHERE A.CONTA_ID IS NULL 

-- PERFIS DUPLICADOS (CONSIDERAR ÚLTIMO)
-- EX_CLIENTES
DELETE [STAGE].[VAGAS_DW].[TMP_CLIENTES] 
FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
WHERE CONTA_ID IN ( SELECT CONTA_ID FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] GROUP BY CONTA_ID HAVING COUNT(*) > 1 )
AND EX_CLIENTE = 1
...
--FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
--WHERE CONTA_ID IN ( SELECT CONTA_ID FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES]  GROUP BY CONTA_ID HAVING COUNT(*) > 1 )
--AND DATA_CONTRATO_MANUT <> ( SELECT MAX(DATA_CONTRATO_MANUT) FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] WHERE CONTA_ID = A.CONTA_ID )
--AND A.TIPO_CLIENTE_MANUT != 'FIT' ;

-- Remover a duplicidade de registros no CUBO de CLIENTES, pelo critério de inatividade do ID_VAGAS: 
--SELECT	SUBQUERY.CONTA_ID , 
--		SUBQUERY.CONTA_CRM , 
--		SUBQUERY.ID_VAGAS , 
--		B.CLIENTE_VAGAS , 
--		B.COD_CLI 
--INTO	#TMP_CLIENTE_VAGAS_VALIDO 
...
--					A1.CONTA_CRM , 
--					A1.ID_VAGAS 
--		  FROM		[VAGAS_DW].[CONTAS_MEMBRO_CRM] AS A1 ) AS SUBQUERY		INNER JOIN [STAGE].[VAGAS_DW].[TMP_CLIENTES] AS B ON SUBQUERY.ID_VAGAS = B.CLIENTE_VAGAS				   
--WHERE	EXISTS ( SELECT	1 
--				 FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A1 
--				 WHERE	SUBQUERY.CONTA_ID = A1.CONTA_ID 
--						AND A1.CONTA_ID IN ( SELECT	AA1.CONTA_ID 
--											 FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS AA1 
--											 GROUP BY 
--													AA1.CONTA_ID 
--											 HAVING COUNT(*) > 1) ) 
--		AND ( B.QTD_EXTRACOES_BCC_6_MESES > 0 
--			  OR B.QTD_LOGIN_ULT_30_DIAS > 0 
--			  OR B.QTD_FUNC_ULT_2_MESES > 0 ) ; 
 
 
 
---- Manter apenas os registros válidos: 
--UPDATE	[STAGE].[VAGAS_DW].[TMP_CLIENTES] 
--SET		CONTA_ID = LOWER(NEWID()) 
--FROM	[STAGE].[VAGAS_DW].[TMP_CLIENTES] AS A 
--WHERE	EXISTS ( SELECT	1 
--				 FROM	#TMP_CLIENTE_VAGAS_VALIDO AS A1 
--				 WHERE	A.CLIENTE_VAGAS = A1.CLIENTE_VAGAS 
--						AND A.COD_CLI <> A1.COD_CLI )  
--		OR EXISTS ( SELECT	1 
--					FROM	#TMP_CLIENTE_VAGAS_VALIDO AS A1 
--					WHERE	A.CLIENTE_VAGAS <> A1.CLIENTE_VAGAS 
--							AND A.COD_CLI <> A1.COD_CLI 
--							AND A.CONTA_ID = A1.CONTA_ID ) ; 



-- ATUALIZAR CLUSTER
--UPDATE [STAGE].[VAGAS_DW].[TMP_CLIENTES] SET CLUSTER = B.CLUSTER
--FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES] A
--INNER JOIN VAGAS_DW.CLUSTER_CLIENTES B ON B.COD_CLI = A.COD_C
...
-- Remove alguma duplicidade que ainda exista
-- Remoção de registros duplicados sem informação de fonte
...
SELECT * FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES]
...
AND A.EX_CLIENTE = 0
AND A.CONTEM_VET = 0
AND EXISTS ( SELECT * FROM [STAGE].[VAGAS_DW].[TMP_CLIENTES_PERFIL_USO] WHERE COD_CLI = A.COD_CLI AND QTD_VAGAS_ETALENT > 0 )
...
FROM	[VAGAS_DW].[CLIENTES] AS A		INNER JOIN STAGE.[VAGAS_DW].[TMP_NUM_PED_FAT] AS B ON A.CONTA_ID = B.CONTA_ID
WHERE	A.EX_CLIENTE = 0
		AND A.CARTEIRA = 'RELACIONAMENTO' ;
...
UPDATE	[VAGAS_DW].[CLIENTES]
SET		CNPJ = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(B.CGC_cli)), CHAR(9), ''), CHAR(10), ''), CHAR(13), ''), '.', ''), '/', ''), '-', ''),
		CLIENTE_VAGAS = B.Ident_cli
FROM	[VAGAS_DW].[CLIENTES] AS A		INNER JOIN [hrh-data].[dbo].[Clientes] AS B ON A.CLIENTE_VAGAS COLLATE SQL_Latin1_General_CP1_CS_AS = B.Ident_cli COLLATE SQL_Latin1_General_CP1_CS_AS
...
-- Atualização do campo QTD_CANDIDATURAS_ULT_12_MESES:
DECLARE	@PRM_COD_HISTCAND INT ;
SET	@PRM_COD_HISTCAND = (	SELECT	MIN(A1.Cod_HistCand)
							FROM	[VAGAS_DW].[CANDIDATURAS] AS A1
							WHERE	A1.DATA_CANDIDATURA >= DATEADD(MONTH, -12, CAST(GETDATE() AS DATE)) ) ;

-- DROP TABLE #TMP_CANDIDATURAS_ULT_12_MESES ;
SELECT	*
INTO	#TMP_CANDIDATURAS_ULT_12_MESES
FROM	[VAGAS_DW].[CANDIDATURAS]
WHERE	Cod_histCand >= @PRM_COD_HISTCAND ;

-- Performance:
CREATE NONCLUSTERED INDEX IDX_#TMP_CANDIDATURAS_ULT_12_MESES_CodVaga_HistCand ON #TMP_CANDIDATURAS_ULT_12_MESES (CodVaga_HistCand) ;

-- DROP TABLE #TMP_CANDIDATURAS_CLIENTE_ULT_12_MESES ;
SELECT	A.COD_CLI ,
		COUNT(*) AS QTD_CANDIDATURAS_ULT_12_MESES
INTO	#TMP_CANDIDATURAS_CLIENTE_ULT_12_MESES
FROM	[VAGAS_DW].[VAGAS] AS A		INNER JOIN #TMP_CANDIDATURAS_ULT_12_MESES AS B ON A.VAGAS_Cod_Vaga = B.CodVaga_HistCand
GROUP BY
		A.COD_CLI ;

UPDATE	[VAGAS_DW].[CLIENTES]
SET		QTD_CANDIDATURAS_ULT_12_MESES = B.QTD_CANDIDATURAS_ULT_12_MESES
FROM	[VAGAS_DW].[CLIENTES] AS A		INNER JOIN #TMP_CANDIDATURAS_CLIENTE_ULT_12_MESES AS B ON A.COD_CLI = B.COD_CLI
WHERE	A.EX_CLIENTE = 0 ;

-- Atualização do campo SEGMENTO_SC:
UPDATE	[VAGAS_DW].[CLIENTES]
SET		SEGMENTO_SC = ISNULL(UPPER(ISNULL(B.SEGMENTO_SC, C.SEGMENTO_SC)), '')
FROM	[VAGAS_DW].[CLIENTES] AS A		LEFT OUTER JOIN [VAGAS_DW].[CONTAS_CRM] AS B ON A.CONTA_ID = B.CONTA_ID
										LEFT OUTER JOIN [VAGAS_DW].[CONTAS_MEMBRO_CRM] AS C ON A.CONTA_ID = C.CONTA_ID ;


-- Campo CONSULTORIA_RH provindo do CRM:
UPDATE	[VAGAS_DW].[CLIENTES]
SET		CONSULTORIA_RH = (SELECT UPPER(A1.CONSULTORIA_RH)
						  FROM	 [VAGAS_DW].[CONTAS_CRM] AS A1
						  WHERE	 A.CONTA_ID = A1.CONTA_ID)
FROM	[VAGAS_DW].[CLIENTES] AS A
WHERE	EXISTS (SELECT	1
...
FROM VAGAS_DW.CLIENTES AS A
	JOIN VAGAS_DW.CONTAS_CRM AS B ON A.CLIENTE_VAGAS = B.ID_VAGAS
*/


-- Atualização dos CNAES com base na tabela dos DADOS ABERTOS DA RECEITA

UPDATE VAGAS_DW.CLIENTES
SET
	CNAE_SECAO_ID = B.SECAO_ID
	,CNAE_SECAO = B.SECAO
	,CNAE_DIVISAO_ID = B.DIVISAO_ID
	,CNAE_DIVISAO = B.DIVISAO
	,CNAE_CLASSE_ID = B.CLASSE_ID
	,CNAE_CLASSE = B.CLASSE
	,CNAE_SUBCLASSE_ID_C = A.CNAE_FISCAL
	,CNAE_SUBCLASSE_DESCR_C = B.SUBCLASSE
FROM
	VAGAS_DW.CLIENTES AS Z
	JOIN VAGAS_DW.CNPJ_CNAE AS A ON REPLACE(TRANSLATE(Z.CNPJ, '.-/','   '),' ','') = A.CNPJ
	LEFT JOIN  [VAGAS_DW].[CNAE_SUBCLASSE] AS B ON A.CNAE_FISCAL = B.SUBCLASSE_ID AND B.VERSAO = '2.2' ;
...
UPDATE VAGAS_DW.CLIENTES SET ATIVO_MANUT = 1
FROM VAGAS_DW.CLIENTES AS A
JOIN [hrh-data].dbo.Clientes as B on A.COD_CLI = B.Cod_cli
WHERE FONTE = 'MANUT'
AND B.EstadoReg_cli > 0
...
UPDATE VAGAS_DW.CLIENTES SET ATIVO_ERP = 1
FROM VAGAS_DW.CLIENTES AS A
	JOIN STAGE.VAGAS_DW.TMP_CONTRATOS_ERP AS B ON A.CONTA_ID = B.CONTA_ID
WHERE B.SITUACAO = 'ATIVO'
	AND B.SEQUENCIA = 1
...
-- Marcar clientes em aviso prévio como clientes ativos
UPDATE VAGAS_DW.CLIENTES SET ATIVO_CRM = 1
FROM VAGAS_DW.CLIENTES A
WHERE  EXISTS ( SELECT 1 FROM VAGAS_DW.CONTAS_CRM WHERE CONTA_ID = A.CONTA_ID AND 
				...
				UNION
				SELECT 1 FROM VAGAS_DW.CONTAS_MEMBRO_CRM WHERE CONTA_ID = A.CONTA_ID AND
					(TIPO IN ('cliente_fit', 'cliente_pet', 'cliente_vagas_redes', 'cliente_em_aviso_previo', 'cliente_recrutador', 'cliente_vagas_redes', 'cliente_flex_anuidade', 'cliente_flex_credito')
					AND CATEGORIA IN ('cliente_fit', 'cliente_pet', 'cliente_vagas_redes', 'cliente_em_aviso_previo', 'cliente_recrutador', 'cliente_vagas_redes', 'cliente_flex_anuidade', 'cliente_flex_credito'))
				)			  