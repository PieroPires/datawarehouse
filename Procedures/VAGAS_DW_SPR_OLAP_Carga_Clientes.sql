...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL 
					,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC,CNAE_SUBCLASSE_ID_C,CNAE_SUBCLASSE_DESCR_C,CNPJ,
					POSVENDA
			  FROM VAGAS_DW.OPORTUNIDADES 
			  WHERE ID_VAGAS COLLATE SQL_Latin1_General_CP1_CI_AS = A.CLIENTE_VAGAS COLLATE SQL_Latin1_General_CP1_CI_AS
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FASE = 'fechado_e_ganho'
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC,CNAE_SUBCLASSE_ID_C,CNAE_SUBCLASSE_DESCR_C,CNPJ,POSVENDA
			    ) B
...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL 
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FASE = 'fechado_e_ganho'
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,CONTA,PERFIL,NEGOCIO,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC,CNAE_SUBCLASSE_ID_C,CNAE_SUBCLASSE_DESCR_C,CNPJ,POSVENDA
			    ) B
WHERE A.EX_CLIENTE =  0 
...
UPDATE VAGAS_DW.TMP_CLIENTES SET MEDIA_Vagas_ULT_3_Meses = CONVERT(FLOAT,A.QTD_Vagas_ULT_3_Meses) / 3 ,
								 ...
								 SALDO_BCE = CONVERT(FLOAT,ISNULL(QTD_BCE,0)) / CONVERT(FLOAT,CAPACIDADE_BCE)
FROM VAGAS_DW.TMP_CLIENTES A
WHERE EX_CLIENTE = 0
...
UPDATE VAGAS_DW.TMP_CLIENTES SET MEDIA_VAGAS_ULT_6_MESES_RE = ISNULL(B.QTD_VAGAS_RE / 6,0),
								 MEDIA_VAGAS_ULT_6_MESES_RI = ISNULL(C.QTD_VAGAS_RI / 6,0),
								 MEDIA_POSICOES_ULT_6_MESES_REDES = ISNULL(D.QTD_POSICOES_REDES / 6,0)
FROM VAGAS_DW.TMP_CLIENTES A
OUTER APPLY ( SELECT COUNT(*) AS QTD_VAGAS_RE
			  FROM VAGAS_DW.VAGAS
			  WHERE COD_CLI = A.COD_CLI
			  AND TIPO_PROCESSO = 'RE'
			  AND DATA_PUBLICACAO >= DATEADD(MONTH,-6,GETDATE()) ) B
OUTER APPLY ( SELECT COUNT(*) AS QTD_VAGAS_RI
			  FROM VAGAS_DW.VAGAS
			  WHERE COD_CLI = A.COD_CLI
			  AND TIPO_PROCESSO = 'RI'
			  AND DATA_PUBLICACAO >= DATEADD(MONTH,-6,GETDATE()) ) C
OUTER APPLY ( SELECT SUM(QTD_POSICOES) AS QTD_POSICOES_REDES
			  FROM VAGAS_DW.VAGAS A1
			  WHERE COD_CLI = A.COD_CLI
			  AND TIPO_PROCESSO = 'REDES'
			  AND DATA_PUBLICACAO >= DATEADD(MONTH,-6,GETDATE())
			  AND QTD_POSICOES < 10000 /* AJUSTE POR CONTA DE ERRO NO MAG. LUIZA */ ) D
WHERE A.EX_CLIENTE = 0

UPDATE VAGAS_DW.TMP_CLIENTES SET STATUS_BCE = NULL,STATUS_VAGAS_RE = NULL,STATUS_VAGAS_RI = NULL,
								 STATUS_CLIENTE = NULL


-- ATUALIZAR STATUS BCE, VAGAS, EXTRAÇÕES
UPDATE VAGAS_DW.TMP_CLIENTES SET STATUS_BCE = CASE WHEN SALDO_BCE < 0.8 THEN 'ABAIXO'
												   WHEN SALDO_BCE >= 0.8 AND SALDO_BCE <= 1 THEN 'REVISÃO'
												   WHEN SALDO_BCE > 1 THEN 'ACIMA' END,
								 STATUS_VAGAS_RE = CASE WHEN MEDIA_VAGAS_ULT_6_MESES_RE + CREDITO_BCC < ISNULL(QTD_MIN_VAGAS,0) 
													    OR MEDIA_VAGAS_ULT_6_MESES_RE = 0 
														THEN 'ABAIXO'
														...
														THEN 'ACIMA' END,
								 STATUS_VAGAS_RI = CASE WHEN QTD_MIN_RI_VAGAS IS NULL OR QTD_MAX_RI_VAGAS IS NULL THEN 'SEM RI'
...
UPDATE VAGAS_DW.TMP_CLIENTES SET STATUS_CLIENTE = CASE WHEN (STATUS_VAGAS_RE = 'ACIMA' OR STATUS_VAGAS_RI = 'ACIMA') AND STATUS_BCE IN ('REVISÃO','ACIMA') 
													   ...
													   WHEN (   (   STATUS_VAGAS_RE = 'ACIMA' 
															    ...
															    OR ( ( STATUS_BCE = 'REVISÃO'
																       AND ( STATUS_VAGAS_RE <> 'ABAIXO' OR STATUS_VAGAS_RI NOT IN ('ABAIXO','SEM RI') ) ) ) ) 
															OR ( STATUS_BCE = 'ACIMA' )
															 )
													   THEN 'IMPORTANTE'
...
OUTER APPLY ( SELECT COUNT(*) AS QTD_VAGAS
			  ...
			  AND A1.TIPO_AJUSTE <> 'Sem Ajuste') B
WHERE DATA_REFERENCIA = ( SELECT MAX(DATA_REFERENCIA) FROM VAGAS_DW.CLIENTES )
AND EX_CLIENTE = 0
