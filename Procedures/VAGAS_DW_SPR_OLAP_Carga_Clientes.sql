...
UPDATE VAGAS_DW.TMP_CLIENTES SET MEDIA_VAGAS_ULT_6_MESES_RE = ISNULL(CONVERT(FLOAT,B.QTD_VAGAS_RE) / 6,0) + CREDITO_BCC , -- CONSIDERAMOS TAMBÉM A EQUIV. DO BCC NA MÉDIA DO RE
...
UPDATE VAGAS_DW.TMP_CLIENTES SET STATUS_BCE = CASE WHEN SALDO_BCE < 0.8 THEN 'ABAIXO'
								 ...
								 STATUS_VAGAS_RE = CASE WHEN MEDIA_VAGAS_ULT_6_MESES_RE  < ISNULL(QTD_MIN_VAGAS,0) 
													    OR MEDIA_VAGAS_ULT_6_MESES_RE = 0 
														THEN 'ABAIXO'
														WHEN MEDIA_VAGAS_ULT_6_MESES_RE >= ISNULL(QTD_MIN_VAGAS,0) 
															 AND MEDIA_VAGAS_ULT_6_MESES_RE <= ISNULL(QTD_MAX_VAGAS,0)
														THEN 'MÉDIA'
														WHEN MEDIA_VAGAS_ULT_6_MESES_RE > ISNULL(QTD_MAX_VAGAS,0) 
														THEN 'ACIMA' END,
