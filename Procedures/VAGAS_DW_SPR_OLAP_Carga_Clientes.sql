...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL 
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FASE = 'fechado_e_ganho'
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC,CNAE_SUBCLASSE_ID_C,CNAE_SUBCLASSE_DESCR_C,CNPJ
			    ) B
...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL 
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FASE = 'fechado_e_ganho'
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,CONTA,PERFIL,NEGOCIO,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC,CNAE_SUBCLASSE_ID_C,CNAE_SUBCLASSE_DESCR_C,CNPJ
			    ) B
WHERE A.EX_CLIENTE =  0 
...
UPDATE VAGAS_DW.TMP_CLIENTES SET STATUS_BCE = CASE WHEN SALDO_BCE < 0.8 THEN 'ABAIXO'
								 ...
								 STATUS_VAGAS_RE = CASE WHEN MEDIA_VAGAS_ULT_6_MESES_RE + CREDITO_BCC < ISNULL(QTD_MIN_VAGAS,0) 
														...
														THEN 'ACIMA' END,
								 STATUS_VAGAS_RI = CASE WHEN MEDIA_VAGAS_ULT_6_MESES_RI < ISNULL(QTD_MIN_RI_VAGAS,0) OR MEDIA_VAGAS_ULT_6_MESES_RI = 0  
														THEN 'ABAIXO'
...
UPDATE VAGAS_DW.TMP_CLIENTES SET STATUS_CLIENTE = CASE WHEN (STATUS_VAGAS_RE = 'ACIMA' OR STATUS_VAGAS_RI = 'ACIMA') AND STATUS_BCE IN ('REVISÃO','ACIMA') 
													   ...
													   WHEN (   (   STATUS_VAGAS_RE = 'ACIMA' 
															    ...
															    OR ( ( STATUS_BCE = 'REVISÃO'
																       AND ( STATUS_VAGAS_RE <> 'ABAIXO' OR STATUS_VAGAS_RI <> 'ABAIXO' ) ) ) ) 
															OR ( STATUS_BCE = 'ACIMA' )
															 )
													   THEN 'IMPORTANTE'
