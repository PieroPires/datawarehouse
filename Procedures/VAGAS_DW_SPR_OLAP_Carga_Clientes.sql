...
-- =============================================
ALTER PROCEDURE VAGAS_DW.SPR_OLAP_Carga_Clientes
...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL 
					,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC
			  FROM VAGAS_DW.OPORTUNIDADES 
			  WHERE ID_VAGAS COLLATE SQL_Latin1_General_CP1_CI_AS = A.CLIENTE_VAGAS COLLATE SQL_Latin1_General_CP1_CI_AS
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FASE = 'fechado_e_ganho'
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC
			    ) B
...
CROSS APPLY ( SELECT CONTAID,CONTAPROPRIETARIO,NEGOCIO,CONTA,PERFIL,SUM(ValorProdutoFINAL) AS ValorProdutoFINAL 
			  ...
			  AND DATAFECHAMENTO = ( SELECT MAX(DATAFECHAMENTO) 
									 ...
									 AND FASE = 'fechado_e_ganho'
									 AND RECORRENTE = 1 )
			 GROUP BY CONTAID,CONTAPROPRIETARIO,CONTA,PERFIL,NEGOCIO,CNAE_SECAO_ID,CNAE_SECAO,CNAE_DIVISAO_ID,CNAE_DIVISAO,CNAE_CLASSE_ID,CNAE_CLASSE,CNAE_FAIXA_FUNCIONARIOS,SEGMENTO_COMERC
			    ) B
WHERE A.EX_CLIENTE =  0 
