...
IF EXISTS ( SELECT * FROM SYS.OBJECTS WHERE NAME = 'SPR_OLTP_Carga_Funil_Candidatos_BCE' AND SCHEMA_NAME(SCHEMA_ID) = 'VAGAS_DW')
DROP PROCEDURE [VAGAS_DW].[SPR_OLTP_Carga_Funil_Candidatos_BCE]
GO
...
CREATE PROCEDURE [VAGAS_DW].[SPR_OLTP_Carga_Funil_Candidatos_BCE]

AS
SET NOCOUNT ON

TRUNCATE TABLE [VAGAS_DW].[TMP_FUNIL_CANDIDATOS_BCE]
...
WHERE	A.REMOVIDO_CAND = 0 -- Currículo não removido
		AND A.TIPO_CADASTRO IN ('Exclusivo de Cliente - BCE') ;
...
SELECT	COD_CAND ,
		DATA_CADASTRO ,
		CASE WHEN TIPO_CADASTRO = 'Exclusivo de Cliente - BCE' THEN 'Currículo Apto' END AS TIPO_CADASTRO ,
		CURRICULO_NOVO ,
		NOME ,
		FOTO ,
		DADOS_PESSOAIS ,
...
INSERT INTO [VAGAS_DW].[TMP_FUNIL_CANDIDATOS_BCE] (COD_CAND, DATA_CADASTRO, TIPO_CADASTRO, CURRICULO_NOVO, NOME, FOTO, DADOS_PESSOAIS, ENDERECO, CONTATO, LINKS_REDES_SOCIAIS, OBJETIVO, AREA_NIVEL_OBJETIVO, PRETENSAO_SALARIAL, REGIAO_INTERESSE, RESUMO_QUALIFIC, NIVEL_ESCOLARIDADE, EXPERIENCIA_PROFISSIONAL, IDIOMA, ULTIMO_SALARIO, INF_COMPLEMENTARES)
SELECT	A.COD_CAND ,
		A.DATA_CADASTRO ,
