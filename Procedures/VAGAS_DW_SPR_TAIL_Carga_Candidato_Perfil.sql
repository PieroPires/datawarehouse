...
-- PRIMEIRO EMPREGO
INSERT INTO VAGAS_DW.TAIL_CANDIDATO_PERFIL (COD_CAND,TIPO_PERFIL)
SELECT DISTINCT A.COD_CAND,15 AS TIPO_PERFIL -- NIVEL_INGLES
FROM VAGAS_DW.CANDIDATOS A
WHERE NOT EXISTS ( SELECT * 
				   FROM VAGAS_DW.EXPERIENCIAS_PROFISSIONAIS
				   WHERE COD_CAND = A.COD_CAND
				   AND DATEDIFF(YEAR,INICIO_EXPERIENCIA,ISNULL(FIM_EXPERIENCIA,GETDATE())) > 1) 
AND A.FEZ_ACESSO_IRRESTRITO = 1
