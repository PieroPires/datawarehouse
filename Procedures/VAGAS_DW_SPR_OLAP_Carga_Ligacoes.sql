...
WHERE EXISTS ( SELECT 1 FROM STAGE.VAGAS_DW.TMP_LIGACOES
				WHERE ID = A.ID )

-- Atualizar nomes de equipes (aquelas que se encontram na tabela DE-PARA VAGAS_DW.EQUIPES_CENTRO_CUSTO_TARIFACAO )
UPDATE STAGE.VAGAS_DW.TMP_LIGACOES SET NOME_EQUIPE = C.NOME
FROM STAGE.VAGAS_DW.TMP_LIGACOES A
...
FROM STAGE.VAGAS_DW.TMP_LIGACOES
