...
WHERE FLAG_ATIVO = 1 AND NOME_CONTROLE = 'EVOLUCAO_CICLO'
	...
	BEGIN
		...
		SET @CMD = 'set PYTHONIOENCODING=cp437 & C:\Python27\python M:\Projetos\Scripts_Python\Exportacao_Arquivos_Google_Drive\LerPlanilhaGoogleDrive.py ' + 
...
-- CASO A DATA JÁ EXISTA APAGAMOS
DELETE FROM VAGAS_DW.EVOLUCAO_MARCOS_EQUIPES WHERE DATA_REFERENCIA = @DATA_REFERENCIA
...
INSERT INTO VAGAS_DW.EVOLUCAO_MARCOS_EQUIPES (NOME_EQUIPE,MARCO_ESTRATEGICO,CRITERIO_SUCESSO,NOME_PLANILHA,URL_PLANILHA,PERC_ATINGIMENTO_MARCO,DATA_REFERENCIA)
...
UPDATE VAGAS_DW.EVOLUCAO_MARCOS_EQUIPES SET 
	   ...
	   PERC_ESPERADO = CASE WHEN REPLACE(REPLACE(A.NOME_PLANILHA,'Ciclo',''),'º','') <> B.CICLO_ATUAL THEN 1
			ELSE CONVERT(NUMERIC(4,2),DATEDIFF(DAY,C.DATA_INICIO,A.DATA_REFERENCIA)) / 
				CONVERT(NUMERIC(4,2),DATEDIFF(DAY,C.DATA_INICIO,ISNULL(D.DATA_INICIO,DATEADD(DAY,60,C.DATA_INICIO)))) END,
	   ...
	   SEMANA_CICLO = CASE WHEN REPLACE(REPLACE(A.NOME_PLANILHA,'Ciclo',''),'º','') <> B.CICLO_ATUAL THEN 'CICLO FECHADO'
						   ELSE 'SEMANA ' + CONVERT(VARCHAR,CONVERT(TINYINT,CONVERT(NUMERIC(4,2),DATEDIFF(DAY,C.DATA_INICIO,A.DATA_REFERENCIA)) / 
										    CONVERT(NUMERIC(4,2),DATEDIFF(DAY,C.DATA_INICIO,D.DATA_INICIO)) 
											* DATEDIFF(WEEK,C.DATA_INICIO,D.DATA_INICIO))) END,
	   -- Calculo de quantas semanas o ciclo possui
	   QTD_SEMANAS_CICLO = DATEDIFF(WEEK,C.DATA_INICIO,D.DATA_INICIO)
FROM VAGAS_DW.EVOLUCAO_MARCOS_EQUIPES A
