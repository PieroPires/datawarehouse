...
------------------- INICIO CAC --------------------------------------------
	...
	-- NOVOS CLIENTES: Registros que existem na tabela do LTV, onde a DATA_REFERENCIA seja a partir de Jan/2017, que não existam há 40 dias atrás da DATA_ATUAL, e que não constem em uma data anterior a atual.
	...
	-- 30/07/2018: Nova regra de extração de novos clientes por mês, a partir do CUBO do LTV.
	SELECT	A.DATA_REFERENCIA ,
			A.MERCADO ,
			A.TIPO_CLIENTE ,
			COUNT(*) AS QTD
	INTO	#TMP_CLIENTES_NOVOS
	FROM	[VAGAS_DW].[LTV] AS A
	WHERE	A.TEMPO_MESES_MERCADO = 0
	GROUP BY
			A.DATA_REFERENCIA ,
			A.MERCADO ,
			A.TIPO_CLIENTE
	...
	UPDATE VAGAS_DW.INDICADORES_CONSOLIDADO SET QTD_CLIENTES_ATIVOS = B.QTD_CLIENTES_ATIVOS,
												...
												QTD_CLIENTES_ATIVOS_REDES = K.QTD_CLIENTES_ATIVOS_REDES ,
												QTD_CLIENTES_NOVOS_REDES = K1.QTD_CLIENTES_NOVOS_REDES ,
												LTV_REDES = K2.LTV_REDES
	...
	OUTER APPLY ( SELECT SUM(QTD) AS QTD_CLIENTES_NOVOS_SUPERA 
				  FROM #TMP_CLIENTES_NOVOS 
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
				  AND YEAR(DATA_REFERENCIA) = YEAR(A.DATA)
				  AND MERCADO = 'SUPERA' ) F
	OUTER APPLY ( SELECT SUM(QTD) AS QTD_CLIENTES_NOVOS_S_MERCADO 
				  FROM #TMP_CLIENTES_NOVOS 
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
				  AND YEAR(DATA_REFERENCIA) = YEAR(A.DATA)
				  AND MERCADO = 'SEM MERCADO' ) F1
	OUTER APPLY ( SELECT SUM(QTD) AS QTD_CLIENTES_NOVOS_SIMPLIFICA 
				  FROM #TMP_CLIENTES_NOVOS 
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
				  AND YEAR(DATA_REFERENCIA) = YEAR(A.DATA)
				  AND MERCADO = 'SIMPLIFICA' ) G
	OUTER APPLY ( SELECT SUM(QTD) AS QTD_CLIENTES_NOVOS 
				  FROM #TMP_CLIENTES_NOVOS 
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
	...
	OUTER APPLY ( SELECT COUNT(*) AS QTD_CLIENTES_ATIVOS_REDES
				  FROM VAGAS_DW.LTV
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
				  AND YEAR(DATA_REFERENCIA) = YEAR(A.DATA)
				  AND TIPO_CLIENTE LIKE '%REDES%' ) K
	OUTER APPLY ( SELECT SUM(QTD) AS QTD_CLIENTES_NOVOS_REDES
				  FROM #TMP_CLIENTES_NOVOS 
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
				  AND YEAR(DATA_REFERENCIA) = YEAR(A.DATA)
				  AND TIPO_CLIENTE LIKE '%REDES%' ) K1
	OUTER APPLY ( SELECT AVG(LTV_MERCADO) AS LTV_REDES
				  FROM VAGAS_DW.LTV
				  WHERE MONTH(DATA_REFERENCIA) = MONTH(A.DATA)
				  AND YEAR(DATA_REFERENCIA) = YEAR(A.DATA)
				  AND TIPO_CLIENTE LIKE '%REDES%' ) K2
	...
	AND A.DATA = EOMONTH(A.DATA) ) -- SEMPRE O ÚLTIMO DIA DO MÊS (MESMA REGRA DO LTV)
	...
	UPDATE VAGAS_DW.INDICADORES_CONSOLIDADO SET CAC_SUPERA = CASE WHEN A.QTD_CLIENTES_NOVOS_SUPERA = 0 
												...
												QTD_CLIENTES_SAIRAM_REDES = ISNULL(A.QTD_CLIENTES_NOVOS_REDES - (A.QTD_CLIENTES_ATIVOS_REDES - B.QTD_CLIENTES_ATIVOS_REDES),0)
	FROM VAGAS_DW.INDICADORES_CONSOLIDADO A
...
UPDATE VAGAS_DW.INDICADORES_CONSOLIDADO SET CHURN_RATE_SUPERA = 
	   ...
	   NEW_CUSTOMER_AQUISITION_SIMPLIFICA = CASE WHEN ISNULL(B.QTD_CLIENTES_ATIVOS_SIMPLIFICA,0) <> 0 
			...
			ELSE NULL END ,
	  CHURN_RATE_REDES = CASE WHEN ISNULL(B.QTD_CLIENTES_ATIVOS_REDES,0) <> 0 
			THEN CONVERT(FLOAT,ISNULL(A.QTD_CLIENTES_SAIRAM_REDES,0)) / CONVERT(FLOAT,ISNULL(B.QTD_CLIENTES_ATIVOS_REDES,0))
			ELSE NULL END ,
	  NEW_CUSTOMER_AQUISITION_REDES = CASE WHEN ISNULL(B.QTD_CLIENTES_ATIVOS_REDES,0) <> 0 
			THEN CONVERT(FLOAT,ISNULL(A.QTD_CLIENTES_NOVOS_REDES,0)) / CONVERT(FLOAT,ISNULL(B.QTD_CLIENTES_ATIVOS_REDES,0))
			ELSE NULL END
FROM VAGAS_DW.INDICADORES_CONSOLIDADO A 
OUTER APPLY ( SELECT TOP 1 * 
			  FROM VAGAS_DW.INDICADORES_CONSOLIDADO 
...
UPDATE VAGAS_DW.INDICADORES_CONSOLIDADO SET NET_CUSTOMER_CHURN_RATE_SUPERA = CHURN_RATE_SUPERA - NEW_CUSTOMER_AQUISITION_SUPERA,
											...
											NET_CUSTOMER_CHURN_RATE_REDES = CHURN_RATE_REDES - NEW_CUSTOMER_AQUISITION_REDES
FROM VAGAS_DW.INDICADORES_CONSOLIDADO A 
WHERE A.FECHAMENTO_MES = 'S'
