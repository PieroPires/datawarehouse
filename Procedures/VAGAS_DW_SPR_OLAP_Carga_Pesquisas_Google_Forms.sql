...
SELECT	
		...
		[1] AS EMAIL ,
		[2] AS SOLICITACAO_ATENDIDA ,
		[3] AS NIVEL_SATISFACAO ,
		IIF([6] = 'Felipe Duarte Concei??o', 'Felipe Duarte Conceição', [6]) AS ATENDENTE ,
		...
		CASE
			WHEN [3] BETWEEN 0 AND 7
				THEN 'INSATISFEITO (0-7)'
			WHEN [3] BETWEEN 8 AND 9
				THEN 'SATISFEITO (8-9)'
			WHEN [3] = 10
...
FROM	[VAGAS_DW].[TMP_PESQUISAS_GOOGLE_FORMS] AS A	
												LEFT OUTER JOIN [VAGAS_DW].[CLIENTES] AS B ON LTRIM(RTRIM(REPLACE(SUBSTRING(REPLACE(REVERSE(SUBSTRING(REVERSE(LTRIM(RTRIM([1]))), 1, CHARINDEX('@', REVERSE(LTRIM(RTRIM([1])))))), '@', ''), 1, CHARINDEX('.', REPLACE(REVERSE(SUBSTRING(REVERSE(LTRIM(RTRIM([1]))), 1, CHARINDEX('@', REVERSE(LTRIM(RTRIM([1])))))), '@', ''))), '.', ''))) COLLATE SQL_Latin1_General_CP1_CI_AS = B.CLIENTE_VAGAS COLLATE SQL_Latin1_General_CP1_CI_AS
