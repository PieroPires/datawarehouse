...
TRUNCATE TABLE [VAGAS_DW].[TMP_CASOS_HISTORICO] ;

INSERT INTO [VAGAS_DW].[TMP_CASOS_HISTORICO](ID_CASO,NUMERO_CASO,VALOR_ANTERIOR,NOVO_VALOR,ALTERADOR,DATA_ALTERACAO_ANTERIOR,DATA_ALTERACAO,ESTAGIO_TRANSICAO)
SELECT	A.ID_CASO ,
		A.NUMERO_CASO ,
		A.VALOR_ANTERIOR ,
		A.NOVO_VALOR ,
		...
		CASE
			WHEN A.ESTAGIO_TRANSICAO = 1 AND B.ORIGEM_SUPORTE = 'Telefone'
				 AND B.DATA_INCLUSAO_NORMALIZADA < ( SELECT	A1.DATA_ALTERACAO
													 FROM	[VAGAS_DW].[TMP_CASOS_HISTORICO] AS A1
		...
		A.DATA_ALTERACAO ,
		A.ESTAGIO_TRANSICAO
FROM	[VAGAS_DW].[TMP_CASOS_HISTORICO] AS A		INNER JOIN [VAGAS_DW].[VAGAS_DW].[CASOS] AS B ON A.ID_CASO COLLATE SQL_Latin1_General_CP1_CI_AI = B.ID_CASO
...
UPDATE	[VAGAS_DW].[TMP_CASOS_HISTORICO]
SET		DATA_ALTERACAO = ISNULL(B.DATA_FECHAMENTO, A.DATA_ALTERACAO)
FROM	[VAGAS_DW].[TMP_CASOS_HISTORICO] AS A		INNER JOIN [VAGAS_DW].[VAGAS_DW].[CASOS] AS B ON A.ID_CASO = B.ID_CASO
WHERE	B.FILA_ATENDIMENTO = 'suporte_empresas'
		AND A.ESTAGIO_TRANSICAO = ( SELECT	MAX(A1.ESTAGIO_TRANSICAO)
									FROM	[VAGAS_DW].[TMP_CASOS_HISTORICO] AS A1		INNER JOIN [VAGAS_DW].[VAGAS_DW].[CASOS] AS A2
