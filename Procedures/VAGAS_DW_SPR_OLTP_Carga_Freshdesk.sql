...
SELECT @ULT_ATUAL_TICKETS = CONVERT(varchar(50), CAST(MAX(DATA_ATUALIZACAO) AT TIME ZONE @TimeZone AT TIME ZONE 'UTC' AS DATETIME), 127)  FROM VAGAS_DW.FRESHDESK_TICKETS

SELECT @ULT_ATUAL_CONTATOS = CONVERT(varchar(50), CAST(MAX(DATA_ATUALIZACAO) AT TIME ZONE @TimeZone AT TIME ZONE 'UTC' AS DATETIME), 127)  FROM VAGAS_DW.FRESHDESK_CONTATOS
