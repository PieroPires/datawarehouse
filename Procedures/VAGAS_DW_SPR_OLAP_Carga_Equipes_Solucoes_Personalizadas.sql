...
--SELECT * FROM VAGAS_DW.TMP_DEMANDAS_SOLUCOES_PERSONALIZADAS
	...
	DELETE FROM VAGAS_DW.TMP_DEMANDAS_SOLUCOES_PERSONALIZADAS_DEMANDAS_MICHELE WHERE ([0] = '' OR [0] IS NULL) OR ([1] = '' OR [1] IS NULL)
	...
	INSERT INTO VAGAS_DW.TMP_DEMANDAS_SOLUCOES_PERSONALIZADAS_DEMANDAS_EQUIPE (RESPONSAVEL_PROJETO,DATA_RECEBIMENTO,COD_DEMANDA,CLIENTE,CATEGORIA,
																			   ...
																			   TEMPO_ORCADO,
																			   DATA_REC_VIAB,DATA_CON_CAN_VIAB,DATA_REC_ORC,DATA_CON_CAN_ORC,COM_VIABILIDADE)

	SELECT [2] AS RESPONSAVEL_PROJETO,
		   CASE WHEN [12] IS NOT NULL AND [12] <> '' 
				THEN CONVERT(SMALLDATETIME,RIGHT([12],4) + -- ANO
				     RIGHT('00' + REPLACE(SUBSTRING([12],CHARINDEX('/',[12])+1,2),'/',''),2) + -- MES
				     RIGHT('00' + REPLACE(SUBSTRING([12],1,CHARINDEX('/',[12])),'/',''),2) ) -- DIA
				ELSE NULL END AS DATA_RECEBIMENTO,
		   [0] AS COD_DEMANDA,
		   [1] AS CLIENTE,
		   NULL AS CATEGORIA,
		   ...
		   1 AS FLAG_DEMANDA,
		   ( CONVERT(FLOAT,REPLACE(SUBSTRING([14],1,CHARINDEX(':',[14])),':','')) * 60 * 60  + -- AS HORA,
		     CONVERT(FLOAT,REPLACE(SUBSTRING([14],CHARINDEX(':',[14]),3),':','')) * 60 + -- AS MINUTOS,
		     CONVERT(FLOAT,REPLACE(REVERSE(SUBSTRING(REVERSE([14]),1,CHARINDEX(':',REVERSE([14])))),':','') ) 
		   ) / 60 / 60 AS TEMPO_GASTO_DSM,
		   CASE WHEN [18] <> '' THEN CONVERT(FLOAT,LTRIM(RTRIM(REPLACE(REPLACE(REPLACE([18],'R$ ',''),'.',''),',','.'))))
				ELSE NULL END AS VALOR_HORA,
		   CASE WHEN [15] <> '' THEN CONVERT(FLOAT,LTRIM(RTRIM(REPLACE(REPLACE(REPLACE([15],'R$ ',''),'.',''),',','.'))))
				ELSE NULL END AS VALOR_ADICIONAL,
		   CASE WHEN [19] <> '' THEN CONVERT(FLOAT,LTRIM(RTRIM(REPLACE(REPLACE(REPLACE([19],'R$ ',''),'.',''),',','.'))))
		  ...
		  CASE WHEN [10] IS NOT NULL AND [10] <> '' 
				...
				ELSE NULL END AS DATA_CON_CAN_ORC,	
	...
	INSERT INTO VAGAS_DW.DEMANDAS_EQUIPES (ID_DEMANDA,NUMERO_DEMANDA,NOME_DEMANDA,NOME_PROJETO,RESPONSAVEL,CRIADOR,TIPO_DEMANDA,PRIORIDADE,STATUS_DEMANDA,DATA_CADASTRAMENTO,
									   ...
									   NOME_DEMANDA_ROOT,NOME_RELEASE,EQUIPE_SOLICITANTE,TIPO_SUB_TAREFA,FLAG_ROOT,CICLO,GRAVIDADE,URGENCIA,ESFORCO,SCORE,MEDIA_DIAS_ENTREGA,
									   DATA_ULT_ENTREGA,DIAS_CONCLUSAO,TIPO_CLIENTE,FATURAVEL,EQUIPE_PROJETO,DATA_PRM_ASSIGNED,FERRAMENTA,SINTOMA,INDICE_SATISFACAO,
									   CLIENTE,VALOR_HORA,TEMPO_GASTO_AUXILIAR,VALOR_ADICIONAL,VALOR_PROPOSTA,VALOR_PROJETO,TEMPO_ORCADO,
									   VALOR_ORCADO,DATA_REC_VIAB,DATA_CON_CAN_VIAB,DATA_REC_ORC,DATA_CON_CAN_ORC,COM_VIABILIDADE)
