...
--SELECT * FROM VAGAS_DW.TMP_DEMANDAS_SOLUCOES_PERSONALIZADAS
	...
	SELECT [4] AS RESPONSAVEL_PROJETO,
		   ...
		   ( CONVERT(FLOAT,REPLACE(SUBSTRING([13],1,CHARINDEX(':',[13])),':','')) * 60 * 60  + -- AS HORA,
		     CONVERT(FLOAT,REPLACE(SUBSTRING([13],CHARINDEX(':',[13]),3),':','')) * 60 + -- AS MINUTOS,
		     CONVERT(FLOAT,REPLACE(REVERSE(SUBSTRING(REVERSE([13]),1,CHARINDEX(':',REVERSE([9])))),':','') ) 
