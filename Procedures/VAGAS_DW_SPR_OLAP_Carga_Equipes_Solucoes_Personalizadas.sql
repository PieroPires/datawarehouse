...
--SELECT * FROM VAGAS_DW.TMP_DEMANDAS_SOLUCOES_PERSONALIZADAS
	...
	SELECT [2] AS RESPONSAVEL_PROJETO,
		   ...
		   1 AS FLAG_DEMANDA,
		   ( CONVERT(FLOAT,REPLACE(SUBSTRING([15],1,CHARINDEX(':',[15])),':','')) * 60 * 60  + -- AS HORA,
		     CONVERT(FLOAT,REPLACE(SUBSTRING([15],CHARINDEX(':',[15]),3),':','')) * 60 + -- AS MINUTOS,
		     CONVERT(FLOAT,REPLACE(REVERSE(SUBSTRING(REVERSE([15]),1,CHARINDEX(':',REVERSE([15])))),':','') ) 
		   ...
		   CASE WHEN [15] <> '' THEN CONVERT(FLOAT,LTRIM(RTRIM(REPLACE(REPLACE(REPLACE([16],'R$ ',''),'.',''),',','.'))))
