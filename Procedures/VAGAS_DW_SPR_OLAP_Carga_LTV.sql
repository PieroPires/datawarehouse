...
CREATE TABLE #TMP_CLIENTES_ATIVOS (COD_CLI INT,CONTA_ID VARCHAR(255) COLLATE SQL_Latin1_General_CP1_CI_AI,CLIENTE_CRM VARCHAR(200) COLLATE SQL_Latin1_General_CP1_CI_AI,
								   ...
								   TEMPO_MESES SMALLINT, LTV MONEY, CLUSTER VARCHAR(25) COLLATE SQL_Latin1_General_CP1_CI_AI, TIPO_CLIENTE VARCHAR(100))
...
SELECT A.COD_CLI,
	   ...
	   AVG(B.VALOR_MENSALIDADE) AS VALOR_MENSALIDADE
FROM VAGAS_DW.CLIENTES A
OUTER APPLY ( SELECT TOP 1	MAX(A1.DataFechamento) AS DATAFECHAMENTO ,
				SUM(A1.ValorProdutoFINAL) AS VALOR_MENSALIDADE
				...
				AND ISNULL(A1.INSERT_MANUAL, 'NÃO') = 'NÃO'
				AND A1.DATAFECHAMENTO <= @DATA_REFERENCIA
				GROUP BY A1.DataFechamento,PRODUTO
				ORDER BY
					A1.DataFechamento DESC ) B
...
SET		TEMPO_MESES_MERCADO = 
						...
						FROM	#TMP_CLIENTES_ATIVOS AS A	OUTER APPLY (SELECT	MIN(A1.DATA_VENCIMENTO) AS DATA_VENCIMENTO
																		 FROM	[VAGAS_DW].[FATURAS] AS A1		INNER JOIN [VAGAS_DW].[GRUPO_PRODUTOS_ERP] AS A2 ON A1.PRODUTO = A2.PRODUTO
																		 WHERE	A.CONTA_ID = A1.COD_CLI_CRM
																				AND A2.GRUPO_I = 'VAGAS'
																				AND A2.GRUPO_II IN ('FIT', 'RECRUTADOR', 'REDES')) AS B
...
INSERT INTO VAGAS_DW.LTV (DATA_REFERENCIA,COD_CLI,CONTA_ID,CLIENTE,TEMPO_MESES_MERCADO,LTV_MERCADO,VALOR_MENSALIDADE,MERCADO, TEMPO_MESES, LTV, CLUSTER, TIPO_CLIENTE)
SELECT @DATA_REFERENCIA, 
	   A.COD_CLI,
	   A.CONTA_ID,
	   A.CLIENTE_CRM AS CLIENTE,
	   A.TEMPO_MESES_MERCADO,
	   A.LTV_MERCADO * 0.8 AS LTV_MERCADO,
	   A.VALOR_MENSALIDADE,
	   A.MERCADO ,
	   A.TEMPO_MESES ,
	   A.LTV * 0.8 AS LTV ,
	   A.CLUSTER ,
	   B.TIPO_CLIENTE
FROM #TMP_CLIENTES_ATIVOS AS A		LEFT OUTER JOIN [VAGAS_DW].[CLIENTES] AS B ON A.CONTA_ID = B.CONTA_ID
