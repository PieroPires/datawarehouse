...
IF EXISTS (SELECT * FROM SYS.OBJECTS WHERE NAME = 'SPR_OLTP_Carga_Links_Transicao_Status_HomePages')
	...
	FROM	(
		SELECT	ID_LINK,
				...
				CASE
					...
					WHEN VALOR_ANTERIOR = 'implantacao' AND VALOR_POSTERIOR = 'implantado' THEN 'IMPLANTACAO -> IMPLANTADO'
					WHEN VALOR_ANTERIOR = 'implantado'  AND VALOR_POSTERIOR = 'publicado' THEN 'IMPLANTADO -> PUBLICADO'
					ELSE UPPER(CONCAT(VALOR_ANTERIOR, ' ', '->', ' ', VALOR_POSTERIOR, ' ', '[CONTRAFLUXO]'))
				END AS FASES
		FROM	[STAGE].[VAGAS_DW].[TMP_LINKS_TRANSICAO_STATUS]) AS SUB
...
INSERT INTO [VAGAS_DW].[VAGAS_DW].[TMP_LINKS_TRANSICAO_STATUS]
SELECT *
FROM	#TMP ;